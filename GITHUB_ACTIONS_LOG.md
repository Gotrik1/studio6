...
- **Стадия `builder`:**
    ...
    3. Копируется `prisma/schema.prisma` бэкенда в соответствующую директорию (`backend/prisma/schema.prisma`).
    4. Запускается `pnpm install` для установки **всех** зависимостей, включая `devDependencies` для всех воркспейсов. Это критично для доступа к `prisma` и `nest` CLI.
    5. **После `pnpm install`** копируется **весь остальной исходный код** (`COPY . .`).
    6. Запускается `pnpm --filter prodvor-backend build` для компиляции TypeScript-кода, который теперь видит сгенерированный на предыдущем шаге Prisma Client.
