...
} from "@/shared/api/genkit/flows/analyze-security-flow";
...
export function SecuritySettingsTab() {
  ...
  const handleAnalyze = async () => {
    ...
    setResult(null);
    try {
      const analysisResult = await analyzeSecurity({
      ...
      console.error(e);
      setError(
        "Не удалось выполнить анализ безопасности. Пожалуйста, попробуйте еще раз.",
      );
    } finally {
      setIsLoading(false);
    }
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Запустите проверку, чтобы AI проанализировал вашу недавнюю активность
          на предмет подозрительных действий и дал рекомендации по защите
          аккаунта.
        </CardDescription>
      </CardHeader>
          ...
          <div className="space-y-4">
            ...
            {result.recommendations.length === 0 ? (
              <Alert variant="default" className="border-green-500/50">
                ...
                <AlertDescription>
                  Никаких подозрительных действий не обнаружено. Ваш аккаунт в
                  безопасности.
                </AlertDescription>
              </Alert>
            ) : (
              result.recommendations.map((rec, index) => (
                <Alert
                  key={index}
                  variant={rec.severity === "high" ? "destructive" : "default"}
                >
