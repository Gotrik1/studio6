    ...
    return (
      <div className="flex h-64 flex-col items-center justify-center rounded-lg border-2 border-dashed p-8 text-center">
        ...
        <p className="text-muted-foreground">
          В этой категории пока нет никого. Попробуйте изменить фильтры.
        </p>
      </div>
    );
  }
            ...
            {users.map((user) => (
              <TableRow key={user.id}>
                ...
                <TableCell className="hidden md:table-cell">
                  ...
                  >
                    {user.status === UserStatus.ACTIVE ? "Активен" : "Забанен"}
                  </Badge>
                </TableCell>
                <TableCell className="text-right">
                  <DropdownMenu>
                    ...
                    <DropdownMenuContent align="end">
                      ...
                      <DropdownMenuSeparator />
                      <DropdownMenuItem
                        onClick={() => onPdAction(user, "credit")}
                      >
                        <Coins className="mr-2 h-4 w-4 text-green-500" />
                        Начислить PD
                      </DropdownMenuItem>
                      <DropdownMenuItem
                        onClick={() => onPdAction(user, "debit")}
                      >
