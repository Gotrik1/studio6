...
const debitReasons = [
  "Штраф за нарушение",
  "Списание за неактивность",
  "Корректировка баланса",
  "Возврат средств",
];

export function UserPdDialog({
  user,
  action,
  isOpen,
  onOpenChange,
  ...
  const handleSave = () => {
    ...
    if (isNaN(numAmount) || numAmount <= 0) {
      toast({
        variant: "destructive",
        title: "Ошибка",
        description: "Введите корректную сумму больше нуля.",
      });
      return;
    }
    if (!reason) {
  ...
  const dialogTitle =
    action === "credit"
      ? `Начислить PD пользователю ${user.name}`
      : `Списать PD у пользователя ${user.name}`;
    ...
    <Dialog open={isOpen} onOpenChange={onOpenChange}>
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>
            Введите сумму и выберите причину для транзакции. Это действие будет
            записано в лог.
          </DialogDescription>
        </DialogHeader>
