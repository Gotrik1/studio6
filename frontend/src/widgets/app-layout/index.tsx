  ...
  const docsNavItems: NavItemWithChildren = {
    ...
    children: [
      ...
      { href: "/documents/architecture", label: "Архитектура" },
      {
        ...
        label: "Бэкенд-документация",
      },
      {
        href: "/documents/backend-roadmap",
        label: "Backend Roadmap",
        icon: FileSignature,
      },
      { href: "/documents/vision-and-principles", label: "Видение проекта" },
      {
        href: "/documents/auth-implementation",
        label: "План Аутентификации",
        icon: FileSignature,
      },
            ...
            <SidebarMenuSub>
              {item.children.map((child) => (
                <SidebarMenuSubItem key={child.href}>
                  <SidebarMenuSubButton
                    href={child.href}
                    variant={isActive(child.href) ? "active" : "default"}
                  >
  ...
  return (
    <>
        ...
        <SidebarContent className="p-2 flex flex-col">
            ...
            <SidebarMenu className="space-y-1">
              ...
              {user.role === "Тренер" && (
                <SidebarMenuItem>
                  <SidebarMenuButton
                    asChild
                    tooltip="Тренерский центр"
                    variant={isActive("/coach-center") ? "active" : "default"}
                  >
                    <Link href="/coach-center">
                      <ClipboardList />
                      {state === "expanded" && <span>Тренерский центр</span>}
                    </Link>
                  </SidebarMenuButton>
                </SidebarMenuItem>
              )}
              ...
              {user.role === "Судья" && (
                <SidebarMenuItem>
                  <SidebarMenuButton
                    asChild
                    tooltip="Центр судейства"
                    variant={isActive("/judge-center") ? "active" : "default"}
                  >
                    <Link href="/judge-center">
                      <Gavel />
                      {state === "expanded" && <span>Центр судейства</span>}
                    </Link>
                  </SidebarMenuButton>
                </SidebarMenuItem>
              )}
          ...
          <div className="pt-2 mt-auto">
            ...
            {user.role === "Администратор" &&
              renderCollapsibleMenu(adminNavItems)}
            {user.role === "Администратор" &&
              renderCollapsibleMenu(docsNavItems)}
      ...
      <SidebarInset className="flex flex-col min-h-screen">
        ...
        <main className="flex-1 overflow-auto p-4 sm:p-6 md:pb-6">
          {children}
        </main>
        <AppFooter />
        <BottomNav />
