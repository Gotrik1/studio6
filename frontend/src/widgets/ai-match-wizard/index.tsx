...
} from "@/shared/api/genkit/flows/analyze-match-challenge-flow";
...
function SuggestionCard({ item, type }: SuggestionCardProps) {
  ...
  const handleActionClick = () => {
    const actionText =
      type === "team" ? "Вызов отправлен" : "Площадка забронирована";
...
export function AiMatchWizard() {
  const [prompt, setPrompt] = useState(
    "Хочу сыграть в футбол в субботу вечером против равной по силе команды.",
  );
  const [isLoading, setIsLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [result, setResult] = useState<AnalyzeMatchChallengeOutput | null>(
    null,
  );
          ...
          <div className="space-y-8 animate-in fade-in-50">
            {result.suggestedTeams.length > 0 && (
              <div>
                <h2 className="font-headline text-2xl font-bold mb-4">
                  Предложенные соперники
                </h2>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {result.suggestedTeams.map((team, i) => (
                    <SuggestionCard key={`team-${i}`} item={team} type="team" />
                  ))}
                </div>
              </div>
            )}
            ...
            {result.suggestedVenues.length > 0 && (
              <div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {result.suggestedVenues.map((venue, i) => (
                    <SuggestionCard
                      key={`venue-${i}`}
                      item={venue}
                      type="venue"
