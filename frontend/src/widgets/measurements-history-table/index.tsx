...
export function MeasurementsHistoryTable({ history }: MeasurementsHistoryTableProps) {
    return (
        <Table>
            <TableHeader>
                <TableRow>
            ...
            <TableBody>
                {history.map(entry => (
                    <TableRow key={entry.id}>
                        <TableCell className="font-medium">{format(new Date(entry.date), 'd MMMM yyyy', { locale: ru })}</TableCell>
                        <TableCell className="text-center font-bold">{entry.weight}</TableCell>
                        <TableCell className="text-center">{entry.bodyFat ?? '-'}</TableCell>
