...
export function ProposeTrainingDialog({
  isOpen,
  onOpenChange,
  challengedPlayer,
  ...
  const onSubmit = async (data: FormValues) => {
    ...
    if (!toId) {
      toast({
        variant: "destructive",
        title: "Ошибка",
        description: "Необходимо выбрать игрока.",
      });
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)}>
            <DialogHeader>
              ...
              <DialogDescription>
                {challengedPlayer
                  ? `Отправить предложение игроку ${challengedPlayer.name}`
                  : "Выберите друга и детали тренировки. Отказ от этого предложения не несет штрафов."}
              </DialogDescription>
            </DialogHeader>
            <div className="grid gap-4 py-4">
              {challengedPlayer ? (
                <FormItem>
                  ...
                  <FormControl>
                    <Input value={challengedPlayer.name} disabled />
                  </FormControl>
                </FormItem>
              ) : (
                <FormField
