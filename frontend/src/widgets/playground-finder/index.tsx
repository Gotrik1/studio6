...
export function PlaygroundFinder() {
  ...
  const [prompt, setPrompt] = useState(
    "Хочу найти тихое футбольное поле с хорошим освещением на вечер",
  );
  const [isLoading, setIsLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [aiSummary, setAiSummary] = useState<string | null>(null);
  const [suggestedPlaygrounds, setSuggestedPlaygrounds] = useState<
    Playground[] | null
  >(null);
  ...
  return (
    <div className="space-y-6">
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            <BrainCircuit /> AI-Поиск площадок
          </CardTitle>
          <CardDescription>
            Опишите своими словами, какое место для игры вы ищете, и наш AI
            подберет лучшие варианты.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <Textarea
      ...
      <div className="mt-8 space-y-4">
        ...
        {!isLoading &&
          suggestedPlaygrounds &&
          suggestedPlaygrounds.length === 0 && (
            <div className="col-span-full text-center py-8 text-muted-foreground">
              <p>
                Подходящих площадок не найдено. Попробуйте уточнить ваш запрос.
              </p>
            </div>
          )}
      </div>
