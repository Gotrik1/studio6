...
import {
  assignMedicalPartner,
  getAssignedMedicalStaff,
  getAvailableMedicalPartners,
  unassignMedicalPartner,
} from "@/entities/tournament/api/medical";
...
export function CrmTournamentMedical({
    ...
    try {
      ...
      if (availableRes.success && Array.isArray(availableRes.data)) {
        const assignedIds = new Set(
          (assignedRes.data as MedicalPartner[]).map(
            (p: MedicalPartner) => p.id,
          ),
        );
        setAvailableMedics(
          (availableRes.data as MedicalPartner[]).filter(
            (p: MedicalPartner) => !assignedIds.has(p.id),
          ),
        );
      } else if (!availableRes.success) {
        throw new Error(
          availableRes.error || "Failed to process available medics",
        );
      }
    } catch (error: unknown) {
      const errorMessage =
      ...
      toast({
        variant: "destructive",
        title: "Ошибка",
        description: `Не удалось загрузить данные: ${errorMessage}`,
      });
    } finally {
      setIsLoading(false);
    }
  ...
  const handleAddMedic = async (medic: MedicalPartner) => {
    ...
    if (result.success) {
      toast({
        title: "Мед. поддержка назначена",
        description: `${medic.name} будет работать на турнире.`,
      });
      await fetchData();
    } else {
      toast({
  ...
  const handleRemoveMedic = async (medic: MedicalPartner) => {
    ...
    if (result.success) {
      toast({
        title: "Мед. поддержка снята",
        description: `${medic.name} снят(а) с турнира.`,
      });
      await fetchData();
    } else {
      toast({
    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Медицинские бригады и специалисты, работающие на этом турнире.
          </CardDescription>
        </CardHeader>
        <CardContent>
          {isLoading ? (
                ...
                {assignedMedics.length > 0 ? (
                    ...
                    <TableRow key={medic.id}>
                      <TableCell>
                        <div className="flex items-center gap-3">
                          <Avatar className="h-9 w-9">
                            <AvatarImage
                              src={medic.avatar || ""}
                              data-ai-hint={medic.avatarHint || "medical logo"}
                            />
                            <AvatarFallback>
                              {medic.name.charAt(0)}
                            </AvatarFallback>
                      ...
                      <TableCell className="text-right">
                        <Button
                          variant="ghost"
                          size="icon"
                          onClick={() => handleRemoveMedic(medic)}
                        >
                  ...
                  <TableRow>
                    <TableCell colSpan={2} className="text-center h-24">
                      Медицинская поддержка не назначена
                    </TableCell>
                  </TableRow>
                )}
              </TableBody>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Доступные для назначения на турнир партнеры.
          </CardDescription>
        </CardHeader>
        <CardContent>
          {isLoading ? (
