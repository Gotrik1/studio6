...
export function TournamentDetailsPage({
  ...
  const handleRegisterTeam = () => {
    startTransition(async () => {
      const result = await registerTeamForTournamentAction(
        tournament.id,
        tournament.slug,
      );
      if (result.success) {
        toast({
          ...
          description: "Ваша команда теперь в списке участников.",
        });
      } else {
        toast({
          variant: "destructive",
        ...
        <div className="relative h-48 sm:h-64">
          ...
          <div className="absolute inset-0 bg-gradient-to-t from-black/80 to-transparent flex items-end p-6">
            <div>
              ...
              <h1 className="font-headline text-2xl sm:text-4xl font-bold text-white shadow-lg mt-2">
                {tournament.name}
              </h1>
            </div>
          </div>
            ...
            <div>
              <Card>
                ...
                <CardContent className="flex items-center gap-3">
                  <Avatar>
                    <AvatarImage
                      src={tournament.organizer.avatar || undefined}
                    />
                    <AvatarFallback>
