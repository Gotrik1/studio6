...
export function AssignSponsorDialog({
  isOpen,
  onOpenChange,
  tournamentId,
  sponsor,
  ...
  const onSubmit = async (data: FormValues) => {
    ...
    if (result.success) {
      toast({
        title: "Спонсор назначен!",
        description: `${sponsor.name} теперь поддерживает турнир.`,
      });
      onSuccess();
      onOpenChange(false);
    } else {
      toast({
        variant: "destructive",
        title: "Ошибка",
        description: result.error || "Не удалось назначить спонсора.",
      });
    }
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)}>
            <DialogHeader>
              ...
              <DialogDescription>
                Укажите сумму спонсорского взноса.
              </DialogDescription>
            </DialogHeader>
            <div className="py-4">
              <FormField
