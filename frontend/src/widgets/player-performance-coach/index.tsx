...
} from "@/shared/api/genkit/flows/analyze-player-performance-flow";
import {
  generateTrainingPlan,
  type GenerateTrainingPlanOutput,
} from "@/shared/api/genkit/flows/generate-training-plan-flow";
...
export function PlayerPerformanceCoach() {
  ...
  const [analysisResult, setAnalysisResult] =
    useState<AnalyzePlayerPerformanceOutput | null>(null);

  const [isGeneratingPlan, setIsGeneratingPlan] = useState(false);
  const [planError, setPlanError] = useState<string | null>(null);
  const [planResult, setPlanResult] =
    useState<GenerateTrainingPlanOutput | null>(null);
  ...
  const handleGetAnalysis = async () => {
    ...
    try {
      ...
      const analysis = await analyzePlayerPerformance({
        ...
        recentWorkouts: JSON.stringify(
          log.filter((l) => l.status === "completed").slice(0, 2),
        ),
      });
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Получите анализ вашей физической подготовки и персональный план на
          неделю.
        </CardDescription>
      </CardHeader>
            ...
            <Card className="bg-primary/5 border-primary/20">
              <CardHeader>
                <CardTitle className="text-lg flex items-center gap-2">
                  <Target className="h-5 w-5 text-primary" /> Фокус на неделю:{" "}
                  {planResult.weeklyFocus}
                </CardTitle>
              </CardHeader>
              <CardContent>
