...
} from "@/shared/api/genkit/flows/find-sponsors-for-team-flow";
...
export function TeamSponsorScout() {
  ...
  const [prompt, setPrompt] = useState(
    "Мы - топ-1 команда платформы, ищем партнеров для участия в городских лигах. Нам нужно финансирование поездок и брендированная форма.",
  );
    ...
    <div className="space-y-6">
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            <Building /> AI-Поиск Спонсоров
          </CardTitle>
          <CardDescription>
            Опишите свою команду, достижения и потребности, и наш AI подберет
            наиболее подходящих спонсоров.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <Textarea
                  ...
                  <div className="flex gap-4">
                    ...
                    <div>
                      <CardTitle className="text-lg hover:underline">
                        <Link href={sponsor.profileUrl}>{sponsor.name}</Link>
                      </CardTitle>
      ...
      {!isLoading && result && result.recommendations.length === 0 && (
        <div className="col-span-full text-center py-8 text-muted-foreground">
          <p>
            Подходящих спонсоров не найдено. Попробуйте уточнить ваш запрос.
          </p>
        </div>
      )}
    </div>
