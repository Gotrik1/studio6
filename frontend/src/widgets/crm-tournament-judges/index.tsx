...
import {
  ...
  getAvailableJudges,
  unassignJudge,
} from "@/entities/tournament/api/judges";
...
export function CrmTournamentJudges({
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      setAvailableJudges(
        availableData.filter((j: User) => !assignedIds.has(j.id)),
      );
    } catch (error: unknown) {
      const errorMessage =
  ...
  const handleAddJudge = async (judge: User) => {
    ...
    if (result.success) {
      toast({
        ...
        description: `${judge.name} был назначен на турнир.`,
      });
      await fetchData();
    } else {
      toast({
        variant: "destructive",
        title: "Ошибка",
        description: "Не удалось назначить судью.",
      });
    }
  };
  ...
  const handleRemoveJudge = async (judge: User) => {
    ...
    if (result.success) {
      toast({
        title: "Судья снят с турнира",
        description: `${judge.name} был удален из судейского состава.`,
      });
      await fetchData();
    } else {
      toast({
        variant: "destructive",
        title: "Ошибка",
        description: "Не удалось снять судью.",
      });
    }
  };
  ...
  const handleEditJudge = (name: string) => {
    toast({
      ...
      description: `Открыт диалог редактирования для судьи ${name}.`,
    });
  };
    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Судейский состав, работающий на этом турнире.
          </CardDescription>
        </CardHeader>
        <CardContent>
          {isLoading ? (
                    ...
                    <TableCell className="text-right">
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end">
                          <DropdownMenuItem
                            onClick={() => handleEditJudge(judge.name)}
                          >
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Аккредитованные судьи, доступные для назначения.
          </CardDescription>
        </CardHeader>
        <CardContent>
          {isLoading ? (
