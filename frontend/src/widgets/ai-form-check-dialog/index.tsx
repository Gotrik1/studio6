...
export function AiFormCheckDialog({
  isOpen,
  onOpenChange,
  exerciseName,
}: AiFormCheckDialogProps) {
  const [isLoading, setIsLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [analysisResult, setAnalysisResult] =
    useState<AnalyzeExerciseFormOutput | null>(null);
  ...
  const handleAnalyze = async () => {
    ...
    reader.onload = async () => {
      ...
      try {
        ...
        console.error(e);
        setError(
          "Не удалось выполнить анализ. Пожалуйста, попробуйте еще раз.",
        );
      } finally {
        setIsLoading(false);
      }
    ...
    <Dialog open={isOpen} onOpenChange={onOpenChangeHandler}>
      <DialogContent className="sm:max-w-lg">
        <DialogHeader>
          ...
          <DialogDescription>
            Загрузите короткое видео (до 15 сек), чтобы AI проанализировал вашу
            технику выполнения.
          </DialogDescription>
        </DialogHeader>
            ...
            <div className="space-y-4 pt-4 border-t animate-in fade-in-50">
              ...
              <div className="space-y-2">
                <h3 className="font-semibold flex items-center gap-2">
                  <XCircle className="h-5 w-5 text-destructive" /> Точки для
                  исправления
                </h3>
              ...
              <div className="space-y-2">
                <h3 className="font-semibold flex items-center gap-2">
                  <CheckCircle className="h-5 w-5 text-green-500" /> Что сделано
                  хорошо
                </h3>
                <ul className="list-disc list-inside text-sm text-muted-foreground space-y-1 pl-2">
                  {analysisResult.positivePoints.map((item, i) => (
                    <li key={`positive-${i}`}>{item}</li>
                  ))}
                </ul>
              </div>
            </div>
