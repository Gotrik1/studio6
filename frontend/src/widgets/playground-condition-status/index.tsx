...
export function PlaygroundConditionStatus({
  status,
  report,
}: PlaygroundConditionStatusProps) {
  if (status === "ok") {
    return (
      <Card>
        <CardHeader>
        ...
        <CardContent>
          <Alert className="border-green-500/50">
            ...
            <AlertDescription>
              Нет активных сообщений о проблемах на этой площадке. Приятной
              игры!
            </AlertDescription>
          </Alert>
        </CardContent>
  ...
  if (status === "issue" && report) {
    return (
      <Card>
        ...
        <CardContent>
          <Alert
            variant={report.severity === "high" ? "destructive" : "default"}
            className={
              report.severity === "medium"
                ? "border-yellow-500/50 text-yellow-500 [&>svg]:text-yellow-500"
                : ""
            }
          >
