...
export function TrainingProposalsWidget() {
  ...
  const incomingProposals = proposals.filter(
    (p) => p.to.id === user.id && p.status === "PENDING",
  );
  const outgoingProposals = proposals.filter(
    (p) => p.from.id === user.id && p.status === "PENDING",
  );
  ...
  if (isLoading) {
    return (
      <Card>
        <CardContent className="p-4">Загрузка...</CardContent>
      </Card>
    );
  }
  ...
  const renderProposal = (
    ...
    const otherUser: ProposalUser =
      type === "incoming" ? proposal.from : proposal.to;
    ...
    return (
      <Card key={proposal.id} className="p-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground">
                {proposal.sport} -{" "}
                {format(new Date(proposal.date), "d MMMM, HH:mm", {
                  locale: ru,
                })}
              </p>
            </div>
          ...
          {type === "incoming" ? (
            <div className="flex gap-1">
              <Button
                ...
                onClick={() =>
                  handleResponse(proposal.id, proposal.from.name, "DECLINED")
                }
                disabled={isPending}
              >
                {isPending ? (
              ...
              <Button
                ...
                onClick={() =>
                  handleResponse(proposal.id, proposal.from.name, "ACCEPTED")
                }
                disabled={isPending}
              >
                {isPending ? (
        ...
        {proposal.program && (
          <p className="text-xs text-muted-foreground mt-2 ml-12 pl-1 border-l-2">
            Программа:{" "}
            <span className="font-medium">{proposal.program.name}</span>
          </p>
        )}
        {proposal.comment && (
          <p className="text-xs text-muted-foreground italic mt-2 ml-12 pl-1 border-l-2">
            «{proposal.comment}»
          </p>
        )}
      </Card>
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Принимайте или отклоняйте предложения от других игроков.
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-4">
        <div>
          <h4 className="font-semibold text-sm mb-2 flex items-center gap-2">
            <ArrowRight className="text-primary" /> Входящие
          </h4>
          <div className="space-y-2">
            {incomingProposals.length > 0 ? (
              incomingProposals.map((p) => renderProposal(p, "incoming"))
            ) : (
        ...
        <div>
          <h4 className="font-semibold text-sm mb-2 flex items-center gap-2">
            <ArrowLeft className="text-muted-foreground" /> Исходящие
          </h4>
          <div className="space-y-2">
            {outgoingProposals.length > 0 ? (
              outgoingProposals.map((p) => renderProposal(p, "outgoing"))
            ) : (
              <p className="text-xs text-muted-foreground text-center p-4">
                Нет отправленных предложений.
              </p>
            )}
          </div>
        </div>
