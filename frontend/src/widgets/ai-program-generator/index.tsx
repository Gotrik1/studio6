...
} from "@/shared/api/genkit/flows/generate-training-program-flow";
...
export function AiProgramGenerator({
  ...
  const onSubmit = async (data: AiFormValues) => {
    ...
    setError(null);
    try {
      const result: GenerateTrainingProgramOutput =
        await generateTrainingProgram(data);
      ...
      setError(
        "Не удалось сгенерировать программу. Попробуйте изменить параметры или повторить попытку позже.",
      );
    } finally {
      setIsLoading(false);
    }
  ...
  return (
    <Card className="max-w-3xl mx-auto">
      <CardHeader>
        ...
        <CardDescription>
          Опишите ваши цели и уровень подготовки, и наш AI-тренер создаст для
          вас персональную программу тренировок.
        </CardDescription>
      </CardHeader>
      <Form {...form}>
          ...
          <CardContent className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    >
                      ...
                      <SelectContent>
                        <SelectItem value="Новичок">
                          Новичок (до 6 мес)
                        </SelectItem>
                        <SelectItem value="Средний">
                          Средний (6-18 мес)
                        </SelectItem>
                        <SelectItem value="Продвинутый">
                          Продвинутый (18+ мес)
                        </SelectItem>
                      </SelectContent>
                    </Select>
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>Тренировок в неделю</FormLabel>
                    <Select
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel>Дополнительный фокус (необязательно)</FormLabel>
                  <FormControl>
                    <Input
                      placeholder="Например: акцент на руки, кардио"
                      {...field}
                    />
                  </FormControl>
                </FormItem>
