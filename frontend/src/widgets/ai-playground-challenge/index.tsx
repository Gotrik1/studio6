...
} from "@/shared/api/genkit/flows/generate-playground-challenge-flow";
...
export function AiPlaygroundChallenge({
  playground,
}: AiPlaygroundChallengeProps) {
  const { toast } = useToast();
  const [result, setResult] =
    useState<GeneratePlaygroundChallengeOutput | null>(null);
  ...
  useEffect(() => {
    const fetchChallenge = async () => {
      ...
      try {
        ...
        console.error(
          `Failed to fetch AI challenge for ${playground.name}:`,
          e,
        );
  ...
  if (error || !result) {
    return (
      <Alert variant="destructive">
        ...
        <AlertDescription>
          {error || "Не удалось загрузить челлендж."}
        </AlertDescription>
      </Alert>
    );
  }
