...
import {
  ...
  unassignSponsor,
} from "@/entities/tournament/api/sponsors";
...
export function CrmTournamentSponsors({
    ...
    try {
      ...
      if (availableRes.success && Array.isArray(availableRes.data)) {
        const assignedIds = new Set(
          assignedData.map((p: SponsorWithAmount) => p.id),
        );
        const adaptedSponsors = (availableRes.data as BackendSponsor[])
          .map(adaptSponsor)
          .filter((s): s is SponsorType => s !== null);
        setAvailableSponsors(
          adaptedSponsors.filter((p) => !assignedIds.has(p.id)),
        );
      } else if (!availableRes.success) {
        throw new Error(
          availableRes.error || "Failed to process available sponsors",
        );
      }
    } catch (error: unknown) {
      const errorMessage =
  ...
  const handleRemoveSponsor = async (sponsor: SponsorType) => {
    ...
    if (result.success) {
      toast({
        title: "Спонсор удален",
        description: `${sponsor.name} больше не является спонсором турнира.`,
      });
      await fetchData();
    } else {
      toast({
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Партнеры, поддерживающие это событие.
            </CardDescription>
          </CardHeader>
          <CardContent>
            {isLoading ? (
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Аккредитованные партнеры, доступные для назначения.
            </CardDescription>
          </CardHeader>
          <CardContent>
            {isLoading ? (
                      ...
                      <TableRow key={sponsor.id}>
                        ...
                        <TableCell className="text-right">
                          <Button
                            variant="outline"
                            size="sm"
                            onClick={() => handleOpenAssignDialog(sponsor)}
                          >
