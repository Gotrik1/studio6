...
export function KingOfTheCourtWidget({
  homeTeamData,
  isLoading,
}: KingOfTheCourtWidgetProps) {
  if (isLoading) {
    return (
      <Card>
        <CardHeader>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            <Crown /> Король площадки
          </CardTitle>
        </CardHeader>
        <CardContent>
          <p className="text-sm text-muted-foreground text-center p-4">
            На этой площадке еще не было матчей. Станьте первыми, кто завоюет
            ее!
          </p>
        </CardContent>
      </Card>
  ...
  return (
    <Card className="bg-amber-500/5 border-amber-500/30">
      <CardHeader>
        <CardTitle className="flex items-center gap-2">
          <Crown className="text-amber-500" /> Король площадки
        </CardTitle>
        <CardDescription>
          Команда с наибольшим количеством побед на этом месте.
        </CardDescription>
      </CardHeader>
      <CardContent className="flex flex-col items-center gap-4 text-center">
              ...
              <TooltipTrigger asChild>
                <div className="relative">
                  <Avatar className="h-20 w-20 border-4 border-amber-400">
                    <AvatarImage
                      src={homeTeamData.logo || undefined}
                      alt={homeTeamData.name}
                      data-ai-hint={homeTeamData.dataAiHint || ""}
                    />
                    <AvatarFallback className="text-2xl">
                      {homeTeamData.name.charAt(0)}
                    </AvatarFallback>
                  </Avatar>
