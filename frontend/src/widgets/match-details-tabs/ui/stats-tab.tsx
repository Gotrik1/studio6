      ...
      <Card>
        ...
        <CardContent>
          <p className="text-sm text-center text-muted-foreground p-8">
            Детальная статистика для этого матча недоступна.
          </p>
        </CardContent>
      </Card>
    );
        ...
        {Object.values(stats).map((value) => (
          <div key={value.label} className="space-y-2">
            ...
            <div className="flex items-center gap-2">
              <Progress
                value={(value.team1 / (value.team1 + value.team2)) * 100}
                className="h-3 [&>div]:bg-primary"
              />
              <Progress
                value={(value.team2 / (value.team1 + value.team2)) * 100}
                className="h-3 scale-x-[-1] [&>div]:bg-destructive"
              />
            </div>
          </div>
        ))}
