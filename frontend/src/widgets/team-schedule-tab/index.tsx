...
import {
  ...
  type FormValues,
} from "@/widgets/schedule-practice-dialog";
import { format } from "date-fns";
import { ru } from "date-fns/locale";
import {
  getTeamPractices,
  createTeamPractice,
} from "@/entities/team/api/practices";
...
export function TeamScheduleTab() {
  ...
  const fetchPractices = useCallback(async () => {
    ...
    if (result.success && Array.isArray(result.data)) {
      setPractices(
        result.data.map((p: BackendPractice) => ({
          id: p.id,
          title: p.title,
  ...
  const handleSchedule = async (data: FormValues) => {
    ...
    if (result.success) {
      ...
      toast({
        variant: "destructive",
        title: "Ошибка",
        description: result.error || "Не удалось запланировать тренировку.",
      });
      return false;
    }
  };
      ...
      <Card>
        <CardHeader className="flex-row items-center justify-between">
          <div>
            ...
            <CardDescription>
              Планируйте и отслеживайте командные тренировки.
            </CardDescription>
          </div>
                ...
                <div className="flex justify-between items-start">
                  <div className="space-y-1">
                    ...
                    <p className="text-sm text-muted-foreground flex items-center gap-2">
                      ...
                      {format(practice.date, "d MMMM yyyy, HH:mm", {
                        locale: ru,
                      })}
                    </p>
                    <p className="text-sm text-muted-foreground flex items-center gap-2">
