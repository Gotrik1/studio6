...
const MatchCard = ({ match }: { match: BracketMatch }) => {
    ...
    return (
      <div className="relative flex flex-col items-center gap-2 rounded-lg border-2 border-amber-400 bg-amber-400/10 p-4 shadow-lg w-52">
        ...
        <div className="text-center">
          <p className="font-headline text-lg font-bold text-amber-500">
            ЧЕМПИОН
          </p>
  ...
  const cardContent = (
    <div
      className={cn(
        "relative flex w-52 flex-col gap-2 rounded-lg border bg-card p-2 text-sm shadow-sm transition-all",
        match.href && "cursor-pointer hover:border-primary hover:shadow-md",
      )}
    >
      {match.team1 && (
        <div
          className={cn(
            ...
            match.score && !team1Wins && "opacity-50",
          )}
        >
          <div className="flex items-center gap-2">
            ...
            <span className={cn("font-medium", team1Wins && "font-bold")}>
              {match.team1.name}
            </span>
          </div>
      ...
      {match.score && <div className="my-1 border-t border-dashed"></div>}
      {match.team2 && (
        <div
          className={cn(
            "flex items-center justify-between transition-opacity",
            match.score && !team2Wins && "opacity-50",
          )}
        >
          <div className="flex items-center gap-2">
            <Avatar className="h-5 w-5">
            ...
            <span className={cn("font-medium", team2Wins && "font-bold")}>
              {match.team2.name}
            </span>
          </div>
