...
export function HolisticAnalysisTab() {
    ...
    const handleGetHolisticAnalysis = async () => {
        ...
        try {
            ...
            console.error(e);
            setHolisticError("Не удалось выполнить комплексный анализ. Пожалуйста, попробуйте еще раз.");
        } finally {
            ...
            <div className="space-y-6">
                <Card>
                    ...
                    <CardContent className="space-y-6">
                        ...
                        {holisticError && <Alert variant="destructive"><AlertCircle className="h-4 w-4" /><AlertTitle>Ошибка</AlertTitle><AlertDescription>{holisticError}</AlertDescription></Alert>}

                        {holisticResult && (
                            <div className="space-y-6 animate-in fade-in-50">
                                <Alert>
                                <AlertTitle>Общая оценка</AlertTitle>
                                <AlertDescription>{holisticResult.overallAssessment}</AlertDescription>
                                </Alert>
                                ...
                                <div className="space-y-2">
                                    ...
                                    <ul className="list-disc list-inside text-sm text-muted-foreground space-y-1 pl-2">
                                        {holisticResult.recommendations.map((item, i) => <li key={`rec-${i}`}>{item}</li>)}
                                    </ul>
                                </div>
                ...
                <Card className="mx-1 border-primary/20 bg-primary/5 shadow-none">
                    <CardHeader>
                        <CardTitle className="flex items-center gap-2"><HeartPulse className="text-primary"/> AI-Проверка техники</CardTitle>
