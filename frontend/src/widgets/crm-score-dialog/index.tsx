...
export function CrmMatchResultDialog({
  isOpen,
  onOpenChange,
  match,
  onMatchUpdate,
}: CrmMatchResultDialogProps) {
  const { toast } = useToast();
  const [resultType, setResultType] = useState<"score" | "technical_defeat">(
    "score",
  );
  const [scoreA, setScoreA] = useState("");
  const [scoreB, setScoreB] = useState("");
  const [techDefeatTeam, setTechDefeatTeam] = useState<string | undefined>(
    undefined,
  );
    ...
    if (resultType === "score") {
      ...
      if (isNaN(numScoreA) || isNaN(numScoreB)) {
        toast({
          variant: "destructive",
          title: "Ошибка",
          description: "Пожалуйста, введите корректный счет.",
        });
      ...
      if (!techDefeatTeam) {
        toast({
          variant: "destructive",
          title: "Ошибка",
          description: "Выберите команду для технического поражения.",
        });
        setIsSubmitting(false);
        return;
      }
      ...
      <DialogContent className="sm:max-w-2xl">
        ...
        <div className="py-4 space-y-4">
          <RadioGroup
            ...
            value={resultType}
            onValueChange={(value) =>
              setResultType(value as "score" | "technical_defeat")
            }
          ...
          {resultType === "score" && (
            <div className="grid grid-cols-2 gap-4 items-center animate-in fade-in-50">
              <div>
                <Label htmlFor="scoreA" className="text-center block mb-2">
                  {match.team1?.name}
                </Label>
                <Input
                  id="scoreA"
              ...
              <div>
                <Label htmlFor="scoreB" className="text-center block mb-2">
                  {match.team2?.name}
                </Label>
                <Input
                  id="scoreB"
          ...
          <div className="space-y-2 pt-2">
            <Label htmlFor="comment">
              Комментарий судьи/организатора (необязательно)
            </Label>
            <Textarea
              id="comment"
              placeholder="Например: Штраф за опоздание, причина технического поражения и т.д."
              value={comment}
