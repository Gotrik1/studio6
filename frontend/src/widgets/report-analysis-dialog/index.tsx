...
} from "@/shared/api/genkit/flows/analyze-report-flow";
...
const getRecommendationText = (rec?: AnalyzeReportOutput["recommendation"]) => {
  switch (rec) {
    case "warning":
      return "Выдать предупреждение";
    case "temp_ban":
      return "Временный бан";
    case "perm_ban":
      return "Постоянный бан";
    case "no_action":
...
export function ReportAnalysisDialog({
  ...
  const handleAnalyzeReport = useCallback(async () => {
    ...
    const mockUserHistory =
      "Пользователь ToxicPlayer123 имеет 2 предыдущих предупреждения за токсичное поведение в чате.";
    ...
    <Dialog open={isOpen} onOpenChange={onOpenChangeHandler}>
      <DialogContent className="sm:max-w-lg">
        <DialogHeader>
          ...
          <DialogDescription>
            Жалоба от <strong>{report.reporter.name}</strong> на{" "}
        ...
        <div className="py-4 space-y-4">
          ...
          <div className="space-y-1">
            ...
            <p className="text-sm p-2 bg-muted rounded-md">
              Пользователь ToxicPlayer123 имеет 2 предыдущих предупреждения за
              токсичное поведение в чате.
            </p>
          </div>
          <Separator />
          <div>
          ...
          <div>
            ...
            <div className="flex flex-col sm:flex-row gap-2">
              <Button
                className="flex-1"
                onClick={() => onResolve(String(report.id), "Предупреждение")}
              >
              ...
              <Button
                className="flex-1"
                variant="destructive"
                onClick={() => onResolve(String(report.id), "Временный бан")}
              >
                Временный бан
              </Button>
              <Button
                className="flex-1"
                variant="destructive"
                onClick={() => onResolve(String(report.id), "Постоянный бан")}
              >
                Постоянный бан
              </Button>
            </div>
        ...
        <DialogFooter>
          <Button variant="ghost" onClick={() => onOpenChangeHandler(false)}>
            Закрыть
          </Button>
        </DialogFooter>
      </DialogContent>
