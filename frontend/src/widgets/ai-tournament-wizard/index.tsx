...
} from "@/shared/api/genkit/flows/generate-tournament-wizard-flow";
...
export function AiTournamentWizard() {
  ...
  const [prompt, setPrompt] = useState(
    "Еженедельный турнир по Valorant для новичков.",
  );
  const [isLoading, setIsLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [result, setResult] = useState<GenerateTournamentWizardOutput | null>(
    null,
  );
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      console.error(e);
      setError(
        "Не удалось сгенерировать концепцию турнира. Попробуйте еще раз.",
      );
    } finally {
      setIsLoading(false);
    }
  ...
  const handleFieldChange = (
    field: keyof Omit<
      ...
      "imageDataUri" | "registrationEndDate" | "tournamentStartDate"
    >,
    value: string,
  ) => {
    if (result) {
      setResult((prev: GenerateTournamentWizardOutput | null) =>
    ...
    <div className="space-y-6">
      <Card className="max-w-3xl mx-auto">
        <CardHeader>
          ...
          <CardDescription>
            Опишите ваш турнир в одном предложении. ИИ сгенерирует название,
            описание, баннер, расписание и призовой фонд.
          </CardDescription>
        </CardHeader>
      ...
      {result && (
        <Card className="max-w-3xl mx-auto animate-in fade-in-50">
          <CardHeader>
            <CardTitle>Шаг 2: Результат</CardTitle>
            <CardDescription>
              Отредактируйте сгенерированные данные и запустите турнир.
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div className="space-y-2">
                ...
                <Select
                  value={result.format}
                  onValueChange={(v) => handleFieldChange("format", v)}
                >
                  <SelectTrigger id="promo-format">
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
