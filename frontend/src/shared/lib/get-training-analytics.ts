...
const getMetrics = (completedWorkouts: TrainingLogEntry[]) => {
  ...
  const monthlyVolume =
    completedWorkouts
      .reduce((sum, entry) => {
        return (
          sum +
          entry.exercises.reduce((exSum, ex) => {
            return (
              exSum +
  ...
  completedWorkouts.forEach((entry) => {
    entry.exercises.forEach((ex) => {
      exerciseCounts.set(
        ex.exercise.name,
        (exerciseCounts.get(ex.exercise.name) || 0) + 1,
      );
    });
  });
  ...
  const favoriteExercise =
    sortedExercises.length > 0 ? sortedExercises[0][0] : "Нет";

  const lastWorkout =
    completedWorkouts.length > 0
      ? [...completedWorkouts].sort(
          (a, b) => new Date(b.date).getTime() - new Date(a.date).getTime(),
        )[0].workoutName || "Тренировка"
      : "Нет";
...
export const getTrainingAnalytics = (log: TrainingLogEntry[]) => {
    ...
    entry.exercises.forEach((exercise) => {
      ...
      exercise.sets.forEach((set) => {
        if (
          ...
          recordHistoryMap[exercise.exercise.name].push({
            date: entry.date,
            e1RM: estimated1RM,
          });
          ...
          if (
            ...
            estimated1RM > bestRecordsMap[exercise.exercise.name].e1RM
          ) {
            bestRecordsMap[exercise.exercise.name] = {
  ...
  const personalRecords = Object.values(bestRecordsMap).sort(
    (a, b) => b.e1RM - a.e1RM,
  );

  for (const key in recordHistoryMap) {
    recordHistoryMap[key].sort(
      (a, b) => new Date(a.date).getTime() - new Date(b.date).getTime(),
    );
  }
  for (const key in fullHistoryMap) {
    fullHistoryMap[key].sort(
      (a, b) => new Date(b.date).getTime() - new Date(a.date).getTime(),
    );
  }
  ...
  return {
    personalRecords,
    recordHistory: recordHistoryMap,
    fullExerciseHistory: fullHistoryMap,
    trainingMetrics,
