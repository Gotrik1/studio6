...
export async function generateSocialMediaPost(
  ...
  const response = await fetchWithAuth<GenerateSocialMediaPostOutput>(
    "/ai/generate-social-media-post",
    {
      method: "POST",
      headers: {
  ...
  if (!response.success || !response.data) {
    ...
    throw new Error(
      response.error || `Backend API responded with status: ${response.status}`,
    );
  }
