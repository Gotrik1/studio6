...
export async function getTeamCoachSummary(
  teamId: string,
): Promise<AnalyzeTeamPerformanceOutput | null> {
  const result = await fetchWithAuth<AnalyzeTeamPerformanceOutput>(
    `/teams/${teamId}/coach-summary`,
  );
  if (!result.success) {
