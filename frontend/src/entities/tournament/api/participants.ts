...
export async function getTournamentApplications(tournamentId: string) {
  const result = await fetchWithAuth<BackendApplication[]>(
    `/teams/${tournamentId}/applications`,
    {
...
export async function getTournamentParticipants(tournamentId: string) {
  const result = await fetchWithAuth<BackendParticipantTeam[]>(
...
export async function approveApplication(
  ...
  const result = await fetchWithAuth<{
    teamId: string;
    team: { slug: string };
  }>(`/team-applications/${applicationId}/accept`, { method: "PATCH" });
  if (result.success) {
...
export async function rejectApplication(
  ...
  const result = await fetchWithAuth<{ teamId: string }>(
    `/team-applications/${applicationId}/decline`,
    { method: "PATCH" },
  );
  if (result.success) {
