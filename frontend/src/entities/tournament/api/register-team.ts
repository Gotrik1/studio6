...
export async function registerTeamForTournamentAction(
  tournamentId: string,
  ...
  try {
    const profileResult = await fetchWithAuth<FullUserProfile>(
    ...
    if (!profileResult.success) {
      return {
        success: false,
        error:
          profileResult.error || "Не удалось получить список ваших команд.",
      };
    }
    ...
    if (!userTeams || userTeams.length === 0) {
      return {
        success: false,
        error:
          "У вас нет команды для регистрации. Сначала создайте или вступите в команду.",
      };
    }
    ...
    const registerResult = await fetchWithAuth(
    ...
    if (!registerResult.success) {
      return {
        success: false,
        error: registerResult.error || "Failed to register team",
      };
    }
