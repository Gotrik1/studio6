...
export async function getSponsorById(id: string): Promise<SponsorDetails | null> {
    const res = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/sponsors/${id}`, { cache: 'no-store' });
    if (!res.ok) {
