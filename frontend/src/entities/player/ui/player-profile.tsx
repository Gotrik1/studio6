...
const CareerTab = dynamic(
  () =>
    import("@/entities/player/ui/player-profile-tabs/career-tab").then(
      (mod) => mod.CareerTab,
    ),
  {
    loading: () => (
...
const AchievementsTab = dynamic(
  () =>
    import("@/entities/player/ui/player-profile-tabs/achievements-tab").then(
      (mod) => mod.AchievementsTab,
    ),
  {
    loading: () => (
      <Card>
...
const TeamsTab = dynamic(
  () =>
    import("@/entities/player/ui/player-profile-tabs/teams-tab").then(
      (mod) => mod.TeamsTab,
    ),
  {
    loading: () => (
...
const GalleryTab = dynamic(
  () =>
    import("@/entities/player/ui/player-profile-tabs/gallery-tab").then(
      (mod) => mod.GalleryTab,
    ),
  {
    loading: () => (
...
const StatsTab = dynamic(
  () =>
    import("@/entities/player/ui/player-profile-tabs/stats-tab").then(
      (mod) => mod.StatsTab,
    ),
  {
    loading: () => (
      ...
      <Card className="overflow-hidden">
        <div className="relative aspect-[3/1] bg-muted/40">
          <Image
            ...
            data-ai-hint="esports gaming background"
          />
                ...
                <div className="flex gap-2 justify-center sm:justify-end">
                  {isCurrentUser ? (
                    <div className="flex gap-2">
                      <Button variant="outline" size="sm" title="Поделиться">
                    ...
                    <div className="flex items-center gap-2">
                      <Button>
                        <UserPlus className="mr-2 h-4 w-4" /> Добавить в друзья
                      </Button>
                      <DropdownMenu>
                  ...
                  <TooltipProvider>
                    <Tooltip>
                      <TooltipTrigger asChild>
                        <Badge
                          variant="outline"
                          className="cursor-help border-amber-500/50"
                        >
                          <Award
                            className={cn("mr-1.5 h-3.5 w-3.5", rank.color)}
                          />
                          {rank.name}
                        </Badge>
                      </TooltipTrigger>
        ...
        <CardContent className="p-6 pt-2 border-b">
          <div className="space-y-2">
            <div className="flex justify-between text-sm text-muted-foreground">
              <span>Прогресс</span>
              <span>
                {(user.xp || 0).toLocaleString("ru-RU")} /{" "}
                {rank.maxPoints === Infinity
                  ? "∞"
                  : rank.maxPoints.toLocaleString("ru-RU")}{" "}
                XP
              </span>
            </div>
        ...
        <div className="p-4 md:p-6">
          <Tabs defaultValue="activity">
            <TabsList className="grid w-full grid-cols-2 md:grid-cols-4 lg:grid-cols-8">
              ...
              <TabsTrigger value="physical-prep">
                <HeartPulse className="mr-2 h-4 w-4" />
                Физ. подготовка
              </TabsTrigger>
              <TabsTrigger value="holistic-analysis">
