...
export function GalleryTab({
  ...
  const handleAddMedia = (item: Omit<GalleryItem, "id">) => {
    setGallery((prev) => [
      { ...item, id: `gallery-item-${Date.now()}` },
      ...prev,
    ]);
  };
      ...
      <Card>
        <CardHeader className="flex flex-row items-center justify-between">
          <div>
            ...
            <CardDescription>
              {isCurrentUser
                ? "Ваши фото и видео с матчей."
                : "Фото и видео с матчей."}
            </CardDescription>
          </div>
          {isCurrentUser && (
          ...
          {gallery.length > 0 ? (
            ...
            <div className="flex h-48 flex-col items-center justify-center rounded-lg border-2 border-dashed p-8 text-center">
              <p className="text-sm text-muted-foreground">
                В вашей галерее пока пусто.
              </p>
              {isCurrentUser && (
                <Button variant="link" onClick={() => setIsUploadOpen(true)}>
                  Загрузите свой первый момент
                </Button>
              )}
            </div>
          )}
