...
export async function getPlayerProfilePageData(
  id: string,
): Promise<PlayerProfilePageData | null> {
...
export async function getPlayerProfile(
  id: string,
): Promise<{ user: FullUserProfile } | null> {
  try {
    ...
    const augmentedProfile: FullUserProfile = {
      ...
      activitySummary: `Пользователь с ${format(
        ...
        "yyyy",
      )} года. Последняя активность: ${formatDistanceToNow(
      ...
      activities: (rawProfile.activities || []).map((act) => ({
        ...act,
        id: String(act.id),
