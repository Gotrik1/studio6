...
export async function getFriendSuggestions(): Promise<FriendSuggestion[]> {
  const result = await fetchWithAuth<FriendSuggestion[]>(
    "/friends/suggestions",
  );
  if (!result.success) return [];
  return result.data;
...
export async function acceptFriendRequest(requestId: string) {
  const result = await fetchWithAuth(`/friends/requests/${requestId}/accept`, {
    method: "POST",
  });
  if (!result.success) throw new Error(result.error);
  revalidatePath("/friends");
...
export async function declineFriendRequest(requestId: string) {
  const result = await fetchWithAuth(`/friends/requests/${requestId}`, {
    method: "DELETE",
  });
  if (!result.success) throw new Error(result.error);
  revalidatePath("/friends");
...
export async function removeFriend(friendId: string) {
  const result = await fetchWithAuth(`/friends/${friendId}`, {
    method: "DELETE",
  });
  if (!result.success) throw new Error(result.error);
  revalidatePath("/friends");
