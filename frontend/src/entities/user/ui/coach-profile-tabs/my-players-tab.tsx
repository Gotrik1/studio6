...
export function MyPlayersTab({ players }: MyPlayersTabProps) {
  ...
  const [selectedPlayer, setSelectedPlayer] = useState<CoachedPlayer | null>(
    null,
  );
    ...
    <>
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Список игроков под вашим руководством.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          {players.map((player) => (
            <Card
              key={player.id}
              className="flex flex-col items-center justify-between gap-4 p-4 sm:flex-row"
            >
