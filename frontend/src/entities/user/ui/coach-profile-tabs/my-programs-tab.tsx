...
export function MyProgramsTab({
  ...
  const coachPrograms = programs.filter(
    (p) => p.author === authorName || p.isAiGenerated,
  );
      ...
      <CardHeader>
        <div className="flex justify-between items-center">
          <div>
            ...
            <CardDescription>
              Созданные вами программы для игроков и команд.
            </CardDescription>
          </div>
          <Button asChild>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <Card key={program.id} className="overflow-hidden flex flex-col">
              ...
              <CardFooter className="p-4 pt-0 mt-auto">
                <Button
                  variant="outline"
                  className="w-full"
                  onClick={() => onAssignProgram(program)}
                >
          ...
          {coachPrograms.length === 0 && (
            <p className="text-sm text-muted-foreground col-span-full text-center py-8">
              Вы еще не создали ни одной программы.
            </p>
          )}
        </div>
      </CardContent>
