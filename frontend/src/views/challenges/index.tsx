...
} from "@/entities/challenge/api/challenges";
...
export function ChallengesPage() {
  ...
  const fetchAllChallenges = useCallback(async () => {
    ...
    try {
      ...
      const errorMessage =
        e instanceof Error ? e.message : "Не удалось загрузить вызовы.";
      toast({
        variant: "destructive",
        title: "Ошибка",
  ...
  const handleCreateChallenge = async (data: CreateChallengeData) => {
    try {
      ...
      toast({
        title: "Вызов брошен!",
        description: "Ваш вызов опубликован и виден другим игрокам.",
      });
      await fetchAllChallenges(); // Refresh all lists
    } catch (err: unknown) {
      const errorMessage =
        err instanceof Error ? err.message : "Не удалось создать вызов.";
      toast({
        variant: "destructive",
        title: "Ошибка",
        description: errorMessage,
  ...
  const handleAcceptChallenge = async (challengeId: string) => {
    ...
    try {
      ...
      toast({
        title: "Вызов принят!",
        description: `Вы приняли вызов от ${challenge?.creator.name}.`,
      });
      await fetchAllChallenges(); // Refresh all lists
    } catch (err: unknown) {
      const errorMessage =
        err instanceof Error ? err.message : "Не удалось принять вызов.";
      toast({
        variant: "destructive",
        title: "Ошибка",
        description: errorMessage,
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="space-y-2">
            ...
            <p className="text-muted-foreground">
              Бросайте публичные вызовы, принимайте чужие и соревнуйтесь за PD и
              славу.
            </p>
          </div>
