...
export async function setHomePlaygroundAction(playgroundId: string): Promise<FetchResult<unknown>> {
  const session = await getSession();
  if (!session?.user) {
    return { success: false, error: 'Вы не авторизованы.', status: 401, data: null };
  }
  const profileResult = await getPlayerProfile(session.user.id);

  if (!profileResult || !profileResult.user) {
     return { success: false, error: 'Не удалось загрузить профиль.', status: 500, data: null };
  }
