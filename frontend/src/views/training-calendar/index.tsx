...
export function TrainingCalendar() {
  ...
  const { log: trainingLogData, isLoading: isTrainingLogLoading } =
    useTraining();
        ...
        <Popover>
                ...
                {dayWorkouts?.map((workout) => (
                  <div
                    ...
                    className={cn(
                      ...
                      statusMap[workout.status].color,
                    )}
                  />
                ))}
          ...
          <PopoverContent className="w-auto p-4 space-y-3">
            <p className="font-semibold text-sm">
              {format(props.date, "d MMMM yyyy", { locale: ru })}
            </p>
            {dayWorkouts?.map((workout) => (
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Наведите курсор на дату, чтобы увидеть детали тренировок и матчей.
        </CardDescription>
      </CardHeader>
