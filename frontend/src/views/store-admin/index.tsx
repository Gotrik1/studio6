...
export function StoreAdminPage() {
    ...
    const onSubmit = (data: FormValues) => {
        ...
        toast({ title: 'Товар добавлен!', description: `"${data.name}" теперь доступен в магазине.` });
        form.reset();
    };
    ...
    return (
        <div className="space-y-6 opacity-0 animate-fade-in-up">
            <div className="space-y-2">
                <h1 className="font-headline text-3xl font-bold tracking-tight">Управление магазином</h1>
                <p className="text-muted-foreground">Добавляйте, редактируйте и удаляйте товары, доступные для покупки.</p>
            </div>
                                    ...
                                    <TableRow key={item.id}>
                                        ...
                                        <TableCell>
                                            <div className="flex items-center gap-1 font-semibold">
                                                {item.isRealMoney ? `$${item.price.toFixed(2)}` : `${item.price} PD`}
                                            </div>
                                        </TableCell>
                        ...
                        <form onSubmit={form.handleSubmit(onSubmit)}>
                            ...
                            <CardContent className="space-y-4">
                                ...
                                <FormField control={form.control} name="price" render={({ field }) => (
                                 ...
                                 <FormField control={form.control} name="isRealMoney" render={({ field }) => (
                                    <FormItem className="flex flex-row items-center justify-between rounded-lg border p-3">
                                        <div className="space-y-0.5"><FormLabel>За реальные деньги?</FormLabel></div>
