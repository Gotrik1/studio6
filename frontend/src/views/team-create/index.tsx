...
} from "@/shared/api/genkit/flows/generate-team-concept-flow";
...
export function NewTeamPage() {
  ...
  const [prompt, setPrompt] = useState(
    "Агрессивная футбольная команда из Москвы, наш символ - волк.",
  );
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      console.error(e);
      setError(
        "Не удалось сгенерировать концепцию команды. Попробуйте еще раз.",
      );
    } finally {
      setIsLoading(false);
    }
  ...
  const handleCreateTeam = async () => {
    ...
    if (createResult.success) {
      ...
      toast({
        ...
        title: "Ошибка",
        description:
          (createResult.error as string) || "Не удалось создать команду.",
      });
    }
    setIsCreating(false);
  };
  ...
  const handleFieldChange = (
    field: keyof GenerateTeamConceptOutput,
    value: string,
  ) => {
    if (result) {
      setResult((prev: GenerateTeamConceptOutput | null) =>
    ...
    <div className="space-y-6 opacity-0 animate-fade-in-up">
      <div className="space-y-2 text-center">
        ...
        <h1 className="font-headline text-3xl font-bold tracking-tight">
          AI-Мастер Создания Команды
        </h1>
        <p className="text-muted-foreground">
          Опишите свою команду, и ИИ создаст название, девиз, логотип и
          описание.
        </p>
      </div>
      ...
      <Card className="max-w-3xl mx-auto">
        <CardHeader>
          ...
          <CardDescription>
            Опишите идею вашей команды в одном предложении.
          </CardDescription>
        </CardHeader>
      ...
      {result && (
        <Card className="max-w-3xl mx-auto animate-in fade-in-50">
          <CardHeader>
            <CardTitle>Шаг 2: Результат</CardTitle>
            <CardDescription>
              Отредактируйте сгенерированные данные и создайте команду.
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
