...
function CrmTournamentParticipantsWidget({
    applications,
    participants,
    isLoading,
    openCollapsibles,
    ...
    return (
        <Tabs defaultValue="applications">
            <TabsList className="grid w-full grid-cols-2">
                ...
                <TabsTrigger value="confirmed">Подтвержденные участники <Badge className="ml-2">{participants.length}</Badge></TabsTrigger>
            </TabsList>
            <TabsContent value="applications" className="mt-4">
                <Card>
                    <CardHeader>
                            ...
                            <Table>
                                <TableHeader>
                                    <TableRow>
                                        ...
                                        <TableHead className="hidden md:table-cell">Капитан</TableHead>
                                ...
                                <TableBody>
                                    {participants.length > 0 ? participants.map(p => (
                                        <Collapsible asChild key={p.id} open={openCollapsibles.includes(p.id)} onOpenChange={() => toggleCollapsible(p.id)}>
                                            <>
                                                                    ...
                                                                    {p.members.map(player => (
                                                                        <TableRow key={player.id}>
                                                                            <TableCell className="font-medium flex items-center gap-2">
                                                                                ...
                                                                                <Link href={`/profiles/player/${player.id}`} className="hover:underline">{player.name}</Link>
                                                                            </TableCell>
                                        ...
                                        <TableRow>
                                            <TableCell colSpan={5} className="text-center h-24">Подтвержденных участников нет</TableCell>
                                        </TableRow>
                                    )}
                                </TableBody>
                            </Table>
...
export function TournamentCrmParticipantsPage({ tournamentId }: { tournamentId: string }) {
    const { toast } = useToast();
