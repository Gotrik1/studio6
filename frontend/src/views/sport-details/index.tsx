...
} from "@/entities/leaderboard/api/get-player-leaderboard";
...
import {
  fetchTournaments,
  type Tournament,
} from "@/entities/tournament/api/get-tournaments";
...
export function SportDetailsPage({ sport }: SportDetailsPageProps) {
    ...
    async function loadData() {
      ...
      try {
        ...
        toast({
          variant: "destructive",
          title: "Ошибка",
          description: "Не удалось загрузить данные для этой дисциплины.",
        });
      } finally {
        setIsLoading(false);
      }
  ...
  const sportLobbies = useMemo(
    () =>
      lobbies.filter((l) =>
        l.sport.toLowerCase().includes(sport.name.toLowerCase()),
      ),
    [sport.name, lobbies],
  );
        ...
        <TabsContent value="overview" className="mt-4 space-y-6">
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Главные соревнования по дисциплине {sport.name}.
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-2">
              {isLoading ? (
                ...
                <p className="text-sm text-muted-foreground text-center p-4">
                  Нет турниров по этой дисциплине.
                </p>
              )}
            </CardContent>
          </Card>
        ...
        <TabsContent value="leaders" className="mt-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Лучшие игроки в дисциплине {sport.name}.
              </CardDescription>
            </CardHeader>
            <CardContent>
              {isLoading ? (
                    ...
                    {topPlayers.map((player) => (
                      <TableRow key={player.rank}>
                        ...
                        <TableCell className="text-right font-semibold">
                          {player.points.toLocaleString("ru-RU")} PD
                        </TableCell>
                      </TableRow>
                    ))}
                  </TableBody>
        ...
        <TabsContent value="teams" className="mt-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Лучшие команды в дисциплине {sport.name}.
              </CardDescription>
            </CardHeader>
            <CardContent>
              {isLoading ? (
              ...
              {!isLoading && topTeams.length === 0 && (
                <p className="text-sm text-center p-4 text-muted-foreground">
                  Пока нет команд в этой дисциплине.
                </p>
              )}
            </CardContent>
          </Card>
        ...
        <TabsContent value="lfg" className="mt-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Открытые лобби для игры в {sport.name}.
              </CardDescription>
            </CardHeader>
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              ...
              {sportLobbies.length === 0 && (
                <p className="text-sm text-center p-4 text-muted-foreground">
                  Нет открытых лобби. Создайте свое!
                </p>
              )}
            </CardContent>
          </Card>
