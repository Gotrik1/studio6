...
import type {
  Application,
  JoinRequest,
} from "@/entities/team-application/model/types";
import { getTeamBySlug, type TeamDetails } from "@/entities/team/api/teams";
import { Skeleton } from "@/shared/ui/skeleton";
import {
  getTeamApplications,
  acceptTeamApplication,
  declineTeamApplication,
} from "@/entities/team-application/api/applications";

const teamNeeds =
  "Мы ищем опытного защитника, который умеет хорошо контролировать поле и начинать атаки. Наш стиль игры - быстрый и комбинационный.";
...
export function TeamManagementPage() {
  ...
  const teamPlayers: CoachedPlayer[] =
    team?.roster.map((p) => ({
      id: p.id,
      name: p.name,
      avatar: p.avatar,
      ...
      if (teamData) {
        ...
        if (appsResult.success && Array.isArray(appsResult.data)) {
          ...
          toast({
            variant: "destructive",
            title: "Ошибка",
            description: "Не удалось загрузить заявки на вступление.",
          });
        }
      }
      setIsLoading(false);
  ...
  const handleAccept = (request: Application) => {
    startTransition(async () => {
      ...
      if (result.success) {
        toast({
          title: "Игрок принят!",
          description: `${request.user.name} теперь в вашей команде.`,
        });
        await fetchData();
      } else {
        toast({
      ...
      <div className="space-y-6 opacity-0 animate-fade-in-up">
        ...
        <Tabs defaultValue="roster">
          <TabsList className="grid w-full grid-cols-8">
            ...
            <TabsTrigger value="smm">
              <Share2 className="mr-2 h-4 w-4" />
              SMM
            </TabsTrigger>
            <TabsTrigger value="training-analytics">
          ...
          <TabsContent value="requests" className="mt-4">
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Игроки, которые хотят присоединиться к вашей команде.
                </CardDescription>
              </CardHeader>
              <CardContent>
                <Table>
