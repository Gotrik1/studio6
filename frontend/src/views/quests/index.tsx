      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {quests.map((quest) => (
          <QuestCard
            ...
            quest={quest}
            onClaim={onClaim}
            claimed={claimedQuests.has(quest.id)}
          />
        ))}
      </div>
    </div>
...
export function QuestsPage() {
  ...
  useEffect(() => {
    getQuests().then((data) => {
      ...
      const initiallyClaimed = new Set(
        data
          .filter(
            (q: Quest) => q.progress >= q.goal && q.type === QuestType.SPECIAL,
          )
          .map((q: Quest) => q.id),
      );
  ...
  const handleResetDailies = () => {
    setQuests((prev) =>
      prev.map((q: Quest) => (q.type === "DAILY" ? { ...q, progress: 0 } : q)),
    );
    ...
    <div className="space-y-8 opacity-0 animate-fade-in-up">
      ...
      <Tabs defaultValue="daily">
        ...
        <TabsContent value="daily" className="mt-6">
          <QuestsSection
            title="Ежедневные"
            quests={quests.filter((q) => q.type === QuestType.DAILY)}
        ...
        <TabsContent value="weekly" className="mt-6">
          <QuestsSection
            title="Еженедельные"
            quests={quests.filter((q) => q.type === QuestType.WEEKLY)}
            onClaim={handleClaimReward}
            claimedQuests={claimedQuests}
          />
        </TabsContent>
        <TabsContent value="special" className="mt-6">
          <QuestsSection
            title="Специальные"
            quests={quests.filter((q) => q.type === QuestType.SPECIAL)}
            onClaim={handleClaimReward}
            claimedQuests={claimedQuests}
          />
        </TabsContent>
