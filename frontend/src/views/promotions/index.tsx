    ...
    useEffect(() => {
        const loadPromotions = async () => {
            ...
            try {
                ...
                console.error(error);
                toast({ variant: 'destructive', title: 'Ошибка', description: 'Не удалось загрузить промо-акции.' });
            } finally {
                setIsLoading(false);
            }
        };
    ...
    return (
        <div className="space-y-6 opacity-0 animate-fade-in-up">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                           ...
                           {promo.sponsor && (
                                <CardHeader>
                                    <div className="flex items-center gap-2">
                                        <Image src={promo.sponsor.logo} alt={promo.sponsor.name} width={24} height={24} className="rounded-full" data-ai-hint="sponsor logo" />
             ...
             {!isLoading && promotions.length === 0 && <p className="text-muted-foreground text-center py-8">Активных промо-акций нет.</p>}
        </div>
    );
}
