...
export function JudgeCenterPage() {
  ...
  const [selectedMatch, setSelectedMatch] = useState<DisputedMatch | null>(
    null,
  );
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      setResolvedMatches(
        finishedData
          .filter((m: Match) => m.resolution)
          .map((m: Match) => ({ ...m, judge: "Вы" }) as ResolvedMatch),
      );
    } catch (error) {
      ...
      toast({
        variant: "destructive",
        title: "Ошибка",
        description: "Не удалось загрузить список матчей.",
      });
    } finally {
      setIsLoading(false);
    }
        ...
        <Tabs defaultValue="queue">
          <TabsList className="grid w-full grid-cols-2">
            <TabsTrigger value="queue">
              Очередь споров{" "}
              <Badge className="ml-2">{disputedMatches.length}</Badge>
            </TabsTrigger>
          ...
          <TabsContent value="queue" className="mt-4">
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Матчи, по которым были открыты споры.
                </CardDescription>
              </CardHeader>
              <CardContent>
                {isLoading ? (
                      ...
                      {disputedMatches.length > 0 ? (
                        disputedMatches.map((match) => (
                          <TableRow key={match.id}>
                            ...
                            <TableCell className="hidden md:table-cell text-muted-foreground truncate max-w-sm">
                              {match.disputeReason}
                            </TableCell>
