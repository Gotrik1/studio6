...
import {
  PlanGameDialog,
  type FormValues as PlanGameFormValues,
} from "@/widgets/plan-game-dialog";
...
import {
  getPlaygroundActivity,
  createCheckIn,
} from "@/entities/playground/api/activity";
import {
  createReview,
  getReviews,
  type CreateReviewData,
  type PlaygroundReview,
} from "@/entities/playground/api/reviews";
...
export default function PlaygroundDetailsPage({
  ...
  initialConditionReport: PlaygroundConditionReport | null;
}) {
  ...
  const [latestIssueReport, setLatestIssueReport] =
    useState<PlaygroundConditionReport | null>(initialConditionReport);
  ...
  const loadSchedule = useCallback(async () => {
    ...
    try {
      ...
      toast({
        variant: "destructive",
        title: "Ошибка",
        description: "Не удалось загрузить расписание площадки.",
      });
    } finally {
  ...
  const handleAddReview = async (reviewData: CreateReviewData) => {
    ...
    if (result.success) {
      toast({
        ...
        description: "Ваш отзыв был опубликован.",
      });
      await loadReviews();
    } else {
      toast({
  ...
  const handleReportSubmit = async (data: {
    ...
    if (result.success && result.data) {
      ...
      toast({
        ...
        title: "Ошибка",
        description:
          "Не удалось отправить отчет. Пожалуйста, попробуйте еще раз.",
      });
    }
  };
      ...
      <div className="space-y-6">
        <Card className="overflow-hidden">
          <div className="relative h-64 w-full">
            <Image
              ...
              data-ai-hint={playground.coverImageHint!}
            />
            <div className="absolute inset-0 bg-gradient-to-t from-black/80 to-transparent" />
            <div className="absolute bottom-6 left-6 text-white">
              <Badge variant="secondary">{playground.type}</Badge>
              <h1 className="font-headline text-4xl font-bold mt-1">
                {playground.name}
              </h1>
              <p className="flex items-center gap-2 mt-1">
                <MapPin className="h-4 w-4" /> {playground.address}
              </p>
            </div>
        ...
        <Tabs defaultValue="info">
          ...
          <TabsContent value="info" className="mt-6">
            <PlaygroundInfoTab
              playground={playground}
              issueReport={latestIssueReport}
            />
          </TabsContent>
          <TabsContent value="schedule" className="mt-6">
            <PlaygroundScheduleTab
              schedule={schedule}
              onPlanClick={openPlanGameDialog}
