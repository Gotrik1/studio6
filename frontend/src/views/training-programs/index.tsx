...
function ProgramCard({
  ...
  isOwner: boolean;
  onSave: (p: TrainingProgram) => Promise<boolean>;
  onDelete: (p: TrainingProgram) => Promise<boolean>;
}) {
  return (
...
export function TrainingProgramsPage() {
  ...
  const handleSaveProgram = async (
    programToClone: TrainingProgram,
  ): Promise<boolean> => {
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div className="space-y-2">
          ...
          <p className="text-muted-foreground">
            Выберите готовую программу от наших экспертов или создайте свою
            собственную.
          </p>
        </div>
        <Button asChild>
        ...
        <TabsContent value="my-programs" className="mt-6">
          {isLoading ? (
            ...
            <p className="text-center text-muted-foreground py-16">
              У вас пока нет программ. Создайте первую или скопируйте из
              сообщества.
            </p>
          )}
        </TabsContent>
