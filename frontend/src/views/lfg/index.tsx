...
import {
  LfgCreateDialog,
  type FormValues as LfgFormValues,
} from "@/widgets/lfg-create-dialog";
import { Textarea } from "@/shared/ui/textarea";
import { findLfgLobbies } from "@/shared/api/genkit/flows/find-lfg-lobbies-flow";
import type {
  LfgLobby as LfgLobbyApiType,
  FindLfgLobbiesOutput,
} from "@/shared/api/genkit/flows/find-lfg-lobbies-flow";
...
export function LfgPage() {
  ...
  const [filteredLobbies, setFilteredLobbies] = useState<
    LfgLobbyEntityType[] | null
  >(null);
  const [typeFilter, setTypeFilter] = useState<"all" | "GAME" | "TRAINING">(
    "all",
  );
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      setFilteredLobbies(
        searchResult.recommendations.map((l: LfgLobbyApiType) => ({
          ...l,
          type: l.type.toUpperCase() as "GAME" | "TRAINING",
    ...
    <>
      <div className="space-y-6 opacity-0 animate-fade-in-up">
          ...
          <div className="space-y-2">
            <div className="flex items-center gap-3">
              ...
              <h1 className="font-headline text-3xl font-bold tracking-tight">
                Поиск игры и тренировок
              </h1>
            </div>
            <p className="text-muted-foreground">
              Найдите компанию для игры или тренировки с помощью AI или создайте
              свое лобби.
            </p>
          </div>
        ...
        <Card className="max-w-3xl mx-auto">
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              <Sparkles className="h-5 w-5 text-primary" /> Умный поиск
            </CardTitle>
            <CardDescription>
              Опишите своими словами, во что, где и когда вы хотите поиграть.
            </CardDescription>
          </CardHeader>
          <CardContent>
            <Textarea
        ...
        <div className="flex justify-center gap-2">
          ...
          <Button
            variant={typeFilter === "TRAINING" ? "default" : "outline"}
