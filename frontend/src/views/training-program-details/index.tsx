...
export function TrainingProgramDetailsPage({
  program,
    ...
    <div className="space-y-6 opacity-0 animate-fade-in-up">
        ...
        <CardContent className="p-6">
          ...
          <div className="mt-4 flex flex-wrap gap-x-6 gap-y-2">
            <div className="flex items-center gap-2">
              ...
              <span>
                Цель: <strong>{program.goal}</strong>
              </span>
            </div>
            <div className="flex items-center gap-2">
              <CalendarDays className="h-4 w-4 text-primary" />{" "}
              <span>
                Частота: <strong>{program.daysPerWeek} дн/нед</strong>
              </span>
            </div>
            <div className="flex items-center gap-2">
              <Dumbbell className="h-4 w-4 text-primary" />{" "}
              <span>
                Тип: <strong>{program.splitType}</strong>
              </span>
            </div>
          </div>
        </CardContent>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Ниже представлена полная структура вашей тренировочной недели.
          </CardDescription>
        </CardHeader>
        <CardContent>
          {program.weeklySplit && program.weeklySplit.length > 0 ? (
                      ...
                      <TableBody>
                        {day.exercises.map((ex) => (
                          <TableRow
                            key={ex.name}
                            className={cn(
                              ex.isSupersetWithPrevious &&
                                "border-t-2 border-dashed border-primary/20",
                            )}
                          >
                            <TableCell className="p-0">
                              {ex.isSupersetWithPrevious && (
                                <Link2 className="h-4 w-4 text-muted-foreground rotate-90 mx-auto" />
                              )}
                            </TableCell>
            ...
            <p className="text-muted-foreground">
              Детальный план для этой программы еще не добавлен.
            </p>
          )}
        </CardContent>
