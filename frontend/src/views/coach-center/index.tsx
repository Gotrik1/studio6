...
export function CoachCenterPage() {
  ...
  const coachedPlayers: CoachedPlayer[] = (coachData.coaching || []).map(
    (player: CoachedPlayerSummary) => ({
      id: player.id,
      name: player.name,
      ...
      adherence:
        player.adherence ?? Math.floor(Math.random() * (98 - 75 + 1) + 75),
      progress: Math.floor(Math.random() * (25 - 5 + 1) + 5),
    }),
  );

  const coachPrograms = programs.filter(
    (p) => p.author === coachData.name || p.isAiGenerated,
  );
    ...
    <>
      <div className="space-y-6 opacity-0 animate-fade-in-up">
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
          ...
          <StatCard
            title="Создано программ"
            value={String(coachPrograms.length)}
            icon={BookOpen}
          />
