...
export function UserManagementPage() {
  ...
  const filteredUsers = useMemo(() => {
    return users.filter((user) => {
      ...
      const matchesStatus =
        statusFilter === "Все статусы" || user.status === statusFilter;
  ...
  const handleConfirmAction = () => {
    ...
    startTransition(() => {
      ...
      const newStatus =
        actionType === "ban" ? UserStatus.BANNED : UserStatus.ACTIVE;
      setUsers((prev) =>
        prev.map((u) =>
      ...
      <AlertDialog open={isAlertOpen} onOpenChange={setIsAlertOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Вы собираетесь {actionType === "ban" ? "забанить" : "разбанить"}{" "}
              пользователя
              <span className="font-bold"> {userToAction?.name}</span>. Это
              действие можно будет отменить.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
