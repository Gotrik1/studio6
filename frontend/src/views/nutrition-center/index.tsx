...
function ProductCatalogTab() {
  ...
  const foodProducts = filteredItems.filter(
    (item) => item.category === "Продукты",
  );
  const supplements = filteredItems.filter(
    (item) => item.category === "Спортивное питание",
  );
      ...
      <Card className="flex flex-col">
        ...
        <CardContent className="p-4 flex-1">
          ...
          <div className="mt-2 text-xs text-muted-foreground space-y-1">
            <p>
              К: {item.calories} | Б: {item.protein} | Ж: {item.fat} | У:{" "}
              {item.carbs}
            </p>
            {item.description && (
              <p className="italic pt-1">{item.description}</p>
            )}
          </div>
        </CardContent>
        <CardFooter>
    ...
    <div className="space-y-6 opacity-0 animate-fade-in-up">
      <div className="space-y-2">
        ...
        <p className="text-muted-foreground">
          Все для управления вашим рационом: от AI-диетолога до подробного
          дневника.
        </p>
      </div>
      ...
      <Tabs defaultValue="ai-nutritionist">
        <TabsList className="grid w-full grid-cols-3">
          <TabsTrigger value="ai-nutritionist">
