    ...
    <div className="space-y-6 opacity-0 animate-fade-in-up">
      <div className="space-y-2">
        ...
        <p className="text-muted-foreground max-w-2xl">
          Узнайте всё о PD — нашей внутренней валюте, которую можно заработать
          за активность и потратить на крутые вещи.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
        ...
        <StatCard
          ...
          value={Math.abs(totalSpent).toLocaleString("ru-RU")}
          icon={TrendingDown}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              <TrendingUp className="text-green-500" /> Как заработать PD?
            </CardTitle>
            <p className="text-sm text-muted-foreground">
              Выполняйте действия на платформе, чтобы пополнять свой баланс.
            </p>
          </CardHeader>
          <CardContent>
        ...
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              <TrendingDown className="text-red-500" /> На что потратить PD?
            </CardTitle>
            <p className="text-sm text-muted-foreground">
              Используйте заработанные PD для покупки товаров и услуг.
            </p>
          </CardHeader>
          <CardContent>
            <ul className="space-y-3">
          ...
          <CardFooter>
            <Button asChild className="w-full">
              <Link href="/store">
                Перейти в магазин <ShoppingCart className="ml-2 h-4 w-4" />
              </Link>
            </Button>
          </CardFooter>
        </Card>
        <Card>
          <CardHeader>
            ...
            <p className="text-sm text-muted-foreground">
              Выполняйте задания, чтобы заработать PD.
            </p>
          </CardHeader>
          <CardContent>
            <ul className="space-y-3">
              {earningRules
                .filter((r) => r.editable === false)
                .slice(0, 5)
                .map((quest) => {
                  return (
                    <li
      ...
      <Card>
        <CardHeader>
          ...
          <p className="text-sm text-muted-foreground">
            Полная история ваших начислений и списаний PD.
          </p>
        </CardHeader>
        <CardContent>
          <Table>
            ...
            <TableBody>
              {history.map((tx) => {
                ...
                return (
                  <TableRow key={tx.id}>
                    <TableCell>
                      {format(new Date(tx.timestamp), "d MMMM yyyy, HH:mm", {
                        locale: ru,
                      })}
                    </TableCell>
                    ...
                    <TableCell
                      className={cn(
                        ...
                        tx.value > 0 ? "text-green-500" : "text-red-500",
                      )}
                    >
                      {tx.value > 0 ? "+" : ""}
                      {tx.value.toLocaleString("ru-RU")} PD
                    </TableCell>
                  </TableRow>
                );
              })}
