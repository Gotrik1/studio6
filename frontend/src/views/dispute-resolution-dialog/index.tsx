...
export function DisputeResolutionDialog({
  ...
  const handleAnalyzeDispute = useCallback(async () => {
    ...
    const mockEvidence = {
      team1Evidence:
        "Chat log shows team 2 player admitting to using a bug. Screenshot of final score: 9-12.",
      team2Evidence:
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onOpenChangeHandler}>
      <DialogContent className="sm:max-w-xl">
        <DialogHeader>
          <DialogTitle>
            Рассмотрение спора: {match.team1.name} vs {match.team2.name}
          </DialogTitle>
          <DialogDescription>
              ...
              <div className="space-y-2">
                ...
                <div className="space-y-2">
                  <Button variant="outline" className="w-full justify-start">
                    <ImageIcon className="mr-2 h-4 w-4" /> Скриншот конца матча
                  </Button>
                  <Button variant="outline" className="w-full justify-start">
                    <MessageSquare className="mr-2 h-4 w-4" /> Лог чата
                  </Button>
                </div>
              </div>
              <div className="space-y-2">
                ...
                <div className="space-y-2">
                  <Button variant="outline" className="w-full justify-start">
                    <ImageIcon className="mr-2 h-4 w-4" /> Видеозапись спорного
                    момента
                  </Button>
                  <Button variant="outline" className="w-full justify-start">
                    <MessageSquare className="mr-2 h-4 w-4" /> Лог чата
            ...
            <div className="flex flex-col sm:flex-row gap-2">
              <Button
                ...
                onClick={() =>
                  onResolve(
                    String(match.id),
                    `победа присуждена ${match.team1.name}`,
                  )
                }
              >
              ...
              <Button
                ...
                onClick={() =>
                  onResolve(
                    String(match.id),
                    `победа присуждена ${match.team2.name}`,
                  )
                }
              >
                <Shield className="mr-2 h-4 w-4" />
              ...
              <Button
                ...
                variant="destructive"
                onClick={() =>
                  onResolve(String(match.id), "назначена переигровка")
                }
              >
                Переигровка
              </Button>
