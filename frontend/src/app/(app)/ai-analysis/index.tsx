...
const mockDisputeData = {
    ...
    disputeReason: "Команда 'Теневые Лисы' обвиняет игрока 'TheWall' в использовании бага карты для получения преимущества.",
...
function AnalysisTools() {
    ...
    const [playerAnalysisResult, setPlayerAnalysisResult] = useState<AnalyzeEsportsPerformanceOutput | null>(null);
    ...
    const [teamAnalysisResult, setTeamAnalysisResult] = useState<AnalyzeTeamPerformanceOutput | null>(null);
    
    // Dispute Analysis State
    const [isDisputeAnalyzing, setIsDisputeAnalyzing] = useState(false);
    const [disputeAnalysisError, setDisputeAnalysisError] = useState<string | null>(null);
    const [disputeAnalysisResult, setDisputeAnalysisResult] = useState<AnalyzeDisputeOutput | null>(null);
    ...
    return (
        <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-8">
            <Card className="flex flex-col">
                <CardHeader>
                    <CardTitle className="flex items-center gap-2"><UserIcon /> Анализ игрока</CardTitle>
                    <CardDescription>Оценка сильных и слабых сторон киберспортсмена.</CardDescription>
                </CardHeader>
                <CardContent className="space-y-4 flex-1">
                    <Button onClick={handlePlayerAnalyze} disabled={isPlayerAnalyzing} className="w-full">
                        {isPlayerAnalyzing ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <Sparkles className="mr-2 h-4 w-4" />}
            ...
            <Card className="flex flex-col">
                <CardHeader>
                    <CardTitle className="flex items-center gap-2"><Users /> Анализ команды</CardTitle>
                    <CardDescription>Комплексный разбор производительности всей команды.</CardDescription>
                </CardHeader>
                <CardContent className="space-y-4 flex-1">
                     <Button onClick={handleTeamAnalyze} disabled={isTeamAnalyzing} className="w-full">
                        {isTeamAnalyzing ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <Sparkles className="mr-2 h-4 w-4" />}
                        Проанализировать (моковые данные)
                    </Button>
                    {isTeamAnalyzing && <Skeleton className="h-40 w-full" />}
                    {teamAnalysisError && <Alert variant="destructive"><AlertTitle>Ошибка</AlertTitle><AlertDescription>{teamAnalysisError}</AlertDescription></Alert>}
                     {teamAnalysisResult && (
                        <div className="space-y-4 pt-4 border-t">
                            <div className="space-y-1"><h4 className="font-semibold text-sm">Фокус на неделю:</h4><p className="font-bold text-primary">{teamAnalysisResult.trainingFocus}</p></div>
                             <div className="space-y-1"><h4 className="font-semibold text-sm">Игрок в фокусе: <span className="font-bold">{teamAnalysisResult.playerInFocus.name}</span></h4><p className="text-sm text-muted-foreground">{teamAnalysisResult.playerInFocus.suggestion}</p></div>
                        </div>
                    )}
                </CardContent>
            </Card>
            ...
            <Card className="flex flex-col">
                <CardHeader>
                    <CardTitle className="flex items-center gap-2"><Gavel /> Анализ спора</CardTitle>
                    <CardDescription>AI-помощник для судей и модераторов.</CardDescription>
                </CardHeader>
                <CardContent className="space-y-4 flex-1">
                    <Button onClick={handleDisputeAnalyze} disabled={isDisputeAnalyzing} className="w-full">
                        {isDisputeAnalyzing ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <Sparkles className="mr-2 h-4 w-4" />}
                        Проанализировать (моковые данные)
                    </Button>
                    {isDisputeAnalyzing && <Skeleton className="h-40 w-full" />}
                    {disputeAnalysisError && <Alert variant="destructive"><AlertTitle>Ошибка</AlertTitle><AlertDescription>{disputeAnalysisError}</AlertDescription></Alert>}
                         ...
                         <div className="space-y-2 pt-4 border-t">
                             <Alert>
                                ...
                                <AlertTitle className="flex justify-between items-center">
                                    ...
                                    <Badge variant="outline" className={cn(getConfidenceColor(disputeAnalysisResult.confidence))}>
