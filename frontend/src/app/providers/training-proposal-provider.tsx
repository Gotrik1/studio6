...
} from "@/entities/training-proposal/api/training-proposals";
...
const TrainingProposalContext = createContext<
  TrainingProposalContextType | undefined
>(undefined);

export const TrainingProposalProvider = ({
  children,
}: {
  children: ReactNode;
}) => {
    ...
    try {
      ...
      if (proposalsResult.success && Array.isArray(proposalsResult.data)) {
        ...
        const formattedProposals = (
          proposalsResult.data as TrainingProposal[]
        ).map((p: TrainingProposal) => ({
          ...p,
  ...
  const addProposal = async (data: {
    ...
    const result = await createTrainingProposal({
      toId: toUserId,
      ...restData,
    });
    if (result.success) {
...
export const useTrainingProposals = () => {
  ...
  if (context === undefined) {
    throw new Error(
      "useTrainingProposals must be used within a TrainingProposalProvider",
    );
  }
  return context;
};
