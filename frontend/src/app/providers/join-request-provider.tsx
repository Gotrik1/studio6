...
const initialRequests: JoinRequest[] = [
  {
    ...
    message:
      "Привет, ищу сильную команду, чтобы выигрывать турниры. Готов играть на любой роли, но предпочитаю нападающего.",
    statsSummary:
      "Опытный нападающий, высокий процент голов, играет в основном в Valorant.",
  },
  {
    id: "req-2",
    teamId: "dvotovyie-atlety",
    ...
    message:
      "Ищу команду для регулярных тренировок и участия в лиге. Хорошо вижу поле.",
    statsSummary:
      "Универсальный игрок, хорошо видит поле. Основная игра - футбол.",
  },
];
...
const JoinRequestContext = createContext<JoinRequestContextType | undefined>(
  undefined,
);
...
export const JoinRequestProvider = ({ children }: { children: ReactNode }) => {
  ...
  const addRequest = (teamId: string, user: User, message: string) => {
    const newRequest: JoinRequest = {
      ...
      message,
      statsSummary:
        "Активный игрок, ищущий новую команду. Готов учиться и вносить вклад в победы.", // Mock summary
    };
...
export const useJoinRequests = () => {
  ...
  if (context === undefined) {
    throw new Error(
      "useJoinRequests must be used within a JoinRequestProvider",
    );
  }
  return context;
};
