...
**Важно:** Текущая конфигурация бэкенда использует глобальный `JwtAuthGuard` для проверки JWT-токенов на каждом защищенном эндпоинте. Это сделано для удобства локальной разработки, так как `AuthService` имитирует Keycloak, а `JwtAuthGuard` — Kong.

Когда приложение будет развернуто за Kong API Gateway, который будет сам проверять токен, этот глобальный гард на бэкенде станет не нужен и может вызывать ошибки (например, двойную проверку).

Чтобы отключить его, достаточно закомментировать или удалить провайдер `APP_GUARD` в файле `src/app.module.ts`:
...
После этого действия бэкенд перестанет самостоятельно проверять JWT и будет доверять запросам, прошедшим через API-шлюз.
