...
**Стратегическое решение для ProDvor:** На текущем этапе мы используем **только Kafka**, чтобы снизить операционную сложность на старте. Kafka покрывает наши самые критические будущие требования (стриминг), и его можно использовать для более простых задач. Однако по мере роста системы, внедрение RabbitMQ для `notification-service` и других сервисов с очередями задач является **логичным следующим шагом** для оптимизации архитектуры.
