...
#### Примеры использования RabbitMQ как очереди задач

| Типовая задача | Реализация в ProDvor с RabbitMQ + NestJS |
|:---|:---|
| **Обработка изображений/видео** | Загрузка файла -> Сообщение `process-media` в очередь -> Отдельный `media-worker` обрабатывает (конвертирует, нарезает превью) и обновляет запись в БД. |
| **Массовая рассылка email** | Администратор запускает рассылку -> Сервис отправляет 1000 сообщений в очередь `email-queue` -> `Email-worker`'ы (с `prefetchCount=5` для rate limiting) забирают и отправляют письма. |
| **Генерация отчетов** | Пользователь запрашивает отчет -> Сообщение `generate-report` в очередь -> `Report-worker` собирает данные, генерирует PDF и отправляет уведомление о готовности. |
| **Сложные вычисления (рейтинг)** | После завершения турнира отправляется задача `recalculate-ratings` -> `Rating-worker` производит вычисления и обновляет данные игроков. |
| **Задачи по расписанию** | Модуль `@nestjs/schedule` (или внешний cron-job) раз в сутки отправляет сообщение `cleanup-old-data` в очередь -> `Cleanup-worker` выполняет задачу. |
