...
import { z } from "zod";
import {
  FindLfgLobbiesInputSchema,
  FindLfgLobbiesOutputSchema,
  LfgLobbySchema,
} from "./schemas/find-lfg-lobbies-schema";
import type {
  FindLfgLobbiesInput,
...
const findLobbiesTool_Backend = ai.defineTool(
  {
    ...
    description:
      "Searches the platform for available game or training lobbies based on a query.",
    inputSchema: z
      .string()
      .describe(
        "A query to filter lobbies, e.g., 'футбол', 'баскетбол в парке горького', 'ищу напарника для тренировки'.",
      ),
        ...
        AND: [
          { endTime: { gt: now } },
          {
            OR: [
              ...
              lowercasedQuery.includes("тренировк") ||
              lowercasedQuery.includes("напарник")
                ? { type: "TRAINING" }
                : {},
