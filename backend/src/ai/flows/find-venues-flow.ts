...
import { z } from "zod";
import {
  FindVenuesInputSchema,
  FindVenuesOutputSchema,
  PlaygroundSchema,
} from "./schemas/find-venues-schema";
import type {
  FindVenuesInput,
  FindVenuesOutput,
} from "./schemas/find-venues-schema";
...
const findAvailableVenuesTool_Backend = ai.defineTool(
  {
    ...
    inputSchema: z.object({
      query: z
        .string()
        .describe(
          "A query describing the desired venue, e.g., 'футбольное поле в Москве', 'бесплатная баскетбольная площадка', 'корт с освещением'.",
        ),
    }),
