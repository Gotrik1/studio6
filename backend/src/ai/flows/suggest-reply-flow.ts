...
const getTeamInfo_Backend = ai.defineTool(
  ...
  async (teamId) => {
    ...
    const scheduleMatches = await prisma.match.findMany({
        where: { status: 'PLANNED', OR: [{ team1Id: teamId }, { team2Id: teamId }] },
        include: { team1: { select: { name: true } }, team2: { select: { name: true } }, tournament: { select: { name: true } } },
    ...
    const matchHistory = historyMatches.map(m => {
        ...
        return { opponent: opponentName, result, score: `${m.team1Score}-${m.team2Score}` };
    });
