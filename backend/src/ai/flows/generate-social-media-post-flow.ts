...
} from "./schemas/generate-social-media-post-schema";
import type {
...
const textContentPrompt = ai.definePrompt({
  ...
  output: {
    schema: z.object({
      ...
      hashtags: z
        .array(z.string())
        .describe("A list of 3-5 relevant hashtags."),
      imagePrompt: z
        .string()
        .describe(
          "A prompt for an image generation model to create a visual for the post.",
        ),
    }),
  },
...
const generateSocialMediaPostFlow_Backend = ai.defineFlow(
  ...
  async (input) => {
    ...
    const { imageDataUri } = await generatePostImage_Backend(
