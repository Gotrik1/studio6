...
export const FindLfgLobbiesInputSchema = z
  .string()
  .describe("A natural language query from a user looking for a game to join.");
export type FindLfgLobbiesInput = z.infer<typeof FindLfgLobbiesInputSchema>;

export const FindLfgLobbiesOutputSchema = z.object({
  recommendations: z
    .array(LfgLobbySchema)
    .describe(
      "A list of up to 5 recommended game lobbies that best match the user's query.",
    ),
});
