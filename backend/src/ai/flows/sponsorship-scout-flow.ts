...
import { z } from "zod";
import {
  SponsorshipScoutInputSchema,
  SponsorshipScoutOutputSchema,
  TeamSchema,
} from "./schemas/sponsorship-scout-schema";
import type {
  SponsorshipScoutInput,
  SponsorshipScoutOutput,
} from "./schemas/sponsorship-scout-schema";
...
const findTeamsTool = ai.defineTool(
  {
    ...
    description:
      "Finds sports teams that are currently looking for sponsorship.",
    inputSchema: z
      .string()
      .describe(
        "A query to filter teams, e.g., 'Football teams', 'teams from Moscow'. Leave empty to get all teams.",
      ),
...
export async function sponsorshipScout(
  input: SponsorshipScoutInput,
