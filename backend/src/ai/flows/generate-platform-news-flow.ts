...
import type { NewsWithAudio } from "./schemas/generate-platform-news-schema";
...
const RecentActivitySchema = z.object({
  topPlayer: z
    .string()
    .describe("The name of the top player on the leaderboards."),
  winningTeam: z
    .object({
      name: z.string(),
      tournament: z.string(),
  ...
  newHotTeam: z
    .string()
    .describe("A newly formed or rapidly rising team.")
    .optional(),
  matchOfTheWeek: z
    .object({
      team1: z.string(),
      ...
      summary: z
        .string()
        .describe("A one-sentence summary of why the match was exciting."),
    })
...
const getRecentActivity = ai.defineTool(
  ...
  async () => {
    ...
    if (lastFinishedMatch) {
      if (
        (lastFinishedMatch.team1Score || 0) >
        (lastFinishedMatch.team2Score || 0)
      ) {
        winningTeam = {
          name: lastFinishedMatch.team1.name,
          tournament:
            lastFinishedMatch.tournament?.name || "Unknown Tournament",
        };
      } else if (
        (lastFinishedMatch.team2Score || 0) >
        (lastFinishedMatch.team1Score || 0)
        ...
        winningTeam = {
          name: lastFinishedMatch.team2.name,
          tournament:
            lastFinishedMatch.tournament?.name || "Unknown Tournament",
        };
      }
    ...
    let matchOfTheWeek:
      | { team1: string; team2: string; score: string; summary: string }
      | undefined;
    if (recentFinishedMatches.length > 0) {
      ...
      matchOfTheWeek = {
        ...
        summary:
          "Невероятно напряженный матч, решившийся на последних секундах.",
      };
    }
...
const newsTextPrompt = ai.definePrompt({
  ...
  prompt:
    "Generate the news digest based on the latest platform activity. The response must be in Russian.",
});
