...
export class TournamentJudgesController {
  ...
  @Post()
  assignJudge(@Param('tournamentId') tournamentId: string, @Body() dto: AssignJudgeDto) {
    return this.service.assignJudge(tournamentId, dto.judgeId);
  }

  @Delete(':judgeId')
  unassignJudge(@Param('tournamentId') tournamentId: string, @Param('judgeId') judgeId: string) {
