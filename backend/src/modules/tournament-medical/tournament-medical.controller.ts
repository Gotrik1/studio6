...
export class TournamentMedicalController {
  ...
  @Post()
  assignMedicalStaff(@Param('tournamentId') tournamentId: string, @Body() dto: AssignMedicalDto) {
    return this.service.assignMedicalStaff(tournamentId, dto.partnerId);
  }

  @Delete(':partnerId')
  unassignMedicalStaff(@Param('tournamentId') tournamentId: string, @Param('partnerId') partnerId: string) {
