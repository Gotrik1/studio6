...
export class TournamentSponsorsService {
  ...
  async getAssignedSponsors(tournamentId: string): Promise<(Sponsor & { amount: number })[]> {
  ...
  async assignSponsor(tournamentId: string, sponsorId: string, amount: number) {
    ...
    this.logger.log(`Assigning sponsor ${sponsor.name} to tournament ${tournament.name} with amount ${amount}`);
