...
export class TrainingProposalsController {
  constructor(private readonly trainingProposalsService: TrainingProposalsService) {}
  ...
  @Get()
  @ApiOperation({ summary: 'Получить все предложения для текущего пользователя' })
  ...
  updateStatus(@Req() req: Request, @Param('id') id: string, @Body() updateDto: UpdateTrainingProposalDto) {
    const userId = (req.user as any).userId;
