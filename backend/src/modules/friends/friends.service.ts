...
export class FriendsService {
  ...
  async acceptRequest(requestId: string, userId: string) {
    ...
    if (!request || request.toId !== userId || request.status !== 'PENDING') {
      throw new NotFoundException('Friend request not found or you are not the recipient.');
    }
  ...
  async declineRequest(requestId: string, userId: string) {
    const request = await this.prisma.friendRequest.findUnique({ where: { id: requestId } });
     if (!request || (request.toId !== userId && request.fromId !== userId)) {
      throw new NotFoundException('Friend request not found or you are not part of this request.');
    }
  ...
  async removeFriend(userId: string, friendId: string) {
     return this.prisma.$transaction(async (tx) => {
       ...
       await tx.friendRequest.deleteMany({
         where: { OR: [{ fromId: userId, toId: friendId }, { fromId: friendId, toId: userId }] }
       });
     });
  }
}
