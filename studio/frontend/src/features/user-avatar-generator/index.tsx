      ...
      <DialogContent className="sm:max-w-md">
        ...
        <Tabs defaultValue="ai" className="w-full">
            ...
            <TabsContent value="upload" className="py-4 space-y-4">
                 <div className="space-y-2">
                    <label htmlFor="avatar-upload" className="flex flex-col items-center justify-center w-full h-64 border-2 border-dashed rounded-lg cursor-pointer bg-muted hover:bg-muted/80">
                        {preview ? (
                            <Image src={preview} alt="Предпросмотр" width={256} height={256} className="h-full w-full object-contain p-2" />
                        ) : (
                        ...
                        <input id="avatar-upload" type="file" className="hidden" accept="image/png, image/jpeg, image/gif" onChange={handleFileChange} />
                    </label>
                </div>
            </TabsContent>
        </Tabs>
        ...
        <DialogFooter>
          ...
          <Button onClick={handleSave} disabled={!generatedAvatar && !preview}>Сохранить аватар</Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  );
