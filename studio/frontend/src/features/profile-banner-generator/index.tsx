      ...
      <DialogContent className="sm:max-w-xl">
        ...
        <Tabs defaultValue="ai" className="w-full">
            ...
            <TabsContent value="upload" className="py-4 space-y-4">
                <div className="space-y-2">
                    <label htmlFor="banner-upload" className="relative flex flex-col items-center justify-center w-full h-48 border-2 border-dashed rounded-lg cursor-pointer bg-muted hover:bg-muted/80">
                        {preview ? (
                            <Image src={preview} alt="Предпросмотр" layout="fill" className="object-cover rounded-lg" />
                        ) : (
                            <div className="flex flex-col items-center justify-center pt-5 pb-6">
                                <UploadCloud className="w-8 h-8 mb-2 text-muted-foreground" />
                                <p className="mb-2 text-sm text-muted-foreground">Нажмите, чтобы загрузить</p>
                                <p className="text-xs text-muted-foreground">PNG, JPG, GIF (16:9)</p>
                            </div>
                        )}
                        <input id="banner-upload" type="file" className="hidden" accept="image/png, image/jpeg, image/gif" onChange={handleFileChange} />
                    </label>
                </div>
            </TabsContent>
        </Tabs>
        <DialogFooter>
          ...
          <Button onClick={handleSave} disabled={!generatedBanner && !preview}>Сохранить фон</Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  );
