        ...
        <TableRow>
            ...
            <TableCell>
                <FormField control={control} name={`exercises.${exerciseIndex}.sets.${setIndex}.loggedReps`} render={({ field }) => (
                    <FormItem><FormControl><Input type="number" {...field} onChange={e => field.onChange(e.target.value === '' ? undefined : parseInt(e.target.value, 10))} value={field.value ?? ''} className="w-20" placeholder={exercise.sets[setIndex]?.plannedReps || '0'} /></FormControl></FormItem>
                )} />
            </TableCell>
            <TableCell>
                <FormField control={control} name={`exercises.${exerciseIndex}.sets.${setIndex}.loggedWeight`} render={({ field }) => (
                    <FormItem><FormControl><Input type="number" {...field} onChange={e => field.onChange(e.target.value === '' ? undefined : parseInt(e.target.value, 10))} value={field.value ?? ''} className="w-20" placeholder={exercise.sets[setIndex]?.plannedWeight?.replace('кг','').trim() || '0'} /></FormControl></FormItem>
                )} />
            </TableCell>
            <TableCell>
...
const ExerciseRow = ({ control, exerciseIndex, exercise, personalRecords, exerciseHistory, allExercises }: ExerciseRowProps) => {
    ...
    const exerciseDetails = allExercises.find(ex => ex.id === exercise.exercise.id);
