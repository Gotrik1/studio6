...
export function StatsTab({ stats }: { stats: PlayerStats | null }) {
    ...
    return (
        <div className="space-y-6">
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-6">
                <StatCard title="Всего матчей" value={String(stats.summary.matches)} icon={Trophy} />
                <StatCard title="Процент побед" value={`${stats.summary.winrate}%`} icon={Shield} />
                <StatCard title="Победная серия" value={String(stats.summary.winStreak)} icon={Flame} />
                <StatCard title="KDA (Valorant)" value={String(stats.summary.kda)} icon={Swords} />
            </div>
