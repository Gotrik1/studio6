...
-   **Основные сервисы (Node.js)**: Набор микросервисов, отвечающих за конкретную бизнес-логику:
    ...
    -   **`notification-service`**: Отправка уведомлений пользователям (WebSocket, push).
-   **Брокер сообщений (NATS / RabbitMQ)**: Асинхронное взаимодействие между сервисами для снижения связанности.
...
| **Брокер сообщений** | NATS / RabbitMQ | Для асинхронных операций (например, нотификации). |
