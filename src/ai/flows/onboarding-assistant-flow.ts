...
export const OnboardingInputSchema = z.object({
  ...
  userRole: z.string().describe("The user's selected role on the platform (e.g., 'Игрок', 'Организатор', 'Капитан')."),
});
...
export const OnboardingOutputSchema = z.object({
  greeting: z.string().describe("A warm, personalized welcome message for the user."),
  mainSuggestion: z.string().describe("A primary call to action or suggestion for the user based on their role."),
  suggestions: z.array(OnboardingSuggestionSchema).describe("An array of 2-3 concrete next steps or 'quests' for the user to complete."),
});
