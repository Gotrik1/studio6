...
import { Search as SearchIcon, Users, Trophy, User, Hash, Gamepad2, Skull } from "lucide-react";
...
export default function SearchPage() {
    ...
    const searchResults = useMemo(() => {
        ...
        const teams = allTeams.filter(t =>
            ...
            t.motto.toLowerCase().includes(lowerQuery)
        );
    ...
    const totalResults = searchResults.users.length + searchResults.teams.length + searchResults.tournaments.length;

    return (
        <div className="space-y-6">
            <div className="space-y-2">
                <h1 className="font-headline text-3xl font-bold tracking-tight">Глобальный поиск</h1>
                <p className="text-muted-foreground">
                    Найдите игроков, команды и турниры по всей платформе.
                </p>
            </div>
            ...
            {query.trim() ? (
                <Tabs defaultValue="all" className="w-full">
                    <TabsList className="grid w-full grid-cols-4">
                        ...
                        <TabsTrigger value="users">Игроки ({searchResults.users.length})</TabsTrigger>
                        <TabsTrigger value="teams">Команды ({searchResults.teams.length})</TabsTrigger>
                        <TabsTrigger value="tournaments">Турниры ({searchResults.tournaments.length})</TabsTrigger>
                    </TabsList>
                    <TabsContent value="all" className="mt-4">
                        {totalResults > 0 ? (
                            <div className="space-y-6">
                                {searchResults.users.length > 0 && <UserResults users={searchResults.users} />}
                                {searchResults.teams.length > 0 && <TeamResults teams={searchResults.teams} />}
                                {searchResults.tournaments.length > 0 && <TournamentResults tournaments={searchResults.tournaments} />}
                            </div>
                        ) : <NoResults />}
                    </TabsContent>
                    <TabsContent value="users" className="mt-4">
                        {searchResults.users.length > 0 ? <UserResults users={searchResults.users} /> : <NoResults />}
                    </TabsContent>
                    <TabsContent value="teams" className="mt-4">
                        {searchResults.teams.length > 0 ? <TeamResults teams={searchResults.teams} /> : <NoResults />}
                    </TabsContent>
                    <TabsContent value="tournaments" className="mt-4">
                         {searchResults.tournaments.length > 0 ? <TournamentResults tournaments={searchResults.tournaments} /> : <NoResults />}
                    </TabsContent>
                </Tabs>
            ) : (
...
const UserResults = ({ users }: { users: typeof userList }) => (
...
const TeamResults = ({ teams }: { teams: typeof allTeams }) => (
    <section className="space-y-4">
...
const TournamentResults = ({ tournaments }: { tournaments: typeof allTournaments }) => (
    <section className="space-y-4">
        <h2 className="font-headline text-2xl font-bold flex items-center gap-2"><Trophy /> Турниры</h2>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {tournaments.map(tournament => (
                 <Link key={tournament.name} href={tournament.slug}>
