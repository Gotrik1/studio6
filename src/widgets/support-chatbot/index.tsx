...
export function SupportChatbot() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const errorMessage: Message = { sender: 'ai', text: 'Произошла ошибка. Пожалуйста, попробуйте позже.' };
                    ...
                    <div className="space-y-4">
                        {messages.map((msg, index) => (
                            <div key={index} className={cn('flex items-start gap-3', msg.sender === 'user' ? 'justify-end' : '')}>
                                ...
                                <div className={cn('max-w-xs rounded-lg px-4 py-2 text-sm shadow-sm', msg.sender === 'ai' ? 'bg-muted' : 'bg-primary text-primary-foreground')}>
                                    {msg.isThinking ? (
