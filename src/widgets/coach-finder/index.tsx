...
function AiRecommendedCoachCard({ coach, reasoning }: { coach: Coach, reasoning: string }) {
...
export function CoachFinder() {
    const { toast } = useToast();
    const [prompt, setPrompt] = useState('Ищу тренера по Valorant для улучшения стрельбы и тактического мышления.');
    ...
    return (
        <div className="space-y-6">
            <Card>
                <CardHeader>
                    <CardTitle className="flex items-center gap-2"><BrainCircuit/> AI-Подбор тренера</CardTitle>
                    <CardDescription>Опишите свои цели, и наш AI подберет для вас наиболее подходящего наставника.</CardDescription>
                </CardHeader>
                <CardContent>
                     <Textarea
