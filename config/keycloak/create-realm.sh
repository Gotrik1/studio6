...
services:
  ...
  keycloak-db:
    ...
    healthcheck:
      test: ["CMD-SHELL", "pg_isready -U ${KC_DB_USERNAME} -d ${KC_DB_DATABASE}"]
      interval: 10s
      timeout: 5s
      retries: 5
    networks:
  ...
  keycloak:
    build:
      context: .
      dockerfile: backend/Dockerfile.keycloak
  ...
  backend:
    ...
    depends_on:
      ...
      keycloak:
        condition: service_healthy
    command: npm run start:prod --workspace=prodvor-backend
    healthcheck:
  ...
  frontend:
    ...
    depends_on:
      ...
      kong-setup:
        condition: service_completed_successfully
    command: npm start --workspace=prodvor-frontend
    logging:
      driver: loki
      options:
        loki-url: "http://localhost:3100/loki/api/v1/push"
